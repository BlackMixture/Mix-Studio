...
function actionIconMarkup(icon) {
  const paths = {
    ...
    extend: '<path d="M3 6h10a2 2 0 0 1 2 2v2h-2V8H5v8h8v-2h2v2a2 2 0 0 1-2 2H3V6Zm13 3 5 3-5 3v-2H9v-2h7V9Z"/>',
...
const maskBoxGeometry = window.MaskBoxGeometry;
let regionDrag = null;
...
function syncRegionSettings(focusPrompt) {
  ...
  const index = region ? state.regions.findIndex((item) => item.id === region.id) : -1;
  const selectedColor = region?.color || REGION_COLORS[(index < 0 ? 0 : index) % REGION_COLORS.length];
  settings.style.setProperty('--selected-region-color', selectedColor);
...
function clearKreaMask(silent) {
  ...
  state.kreaMaskBoxes = [];
  state.kreaMaskActiveBoxId = null;
...
function hasEditMask() {
  return !!state.kreaMask || state.kreaMaskDirty || !!state.kreaMaskPreview || state.kreaMaskBoxes.length > 0;
}
...
function renderKreaMaskTools() {
  ...
  const boxCount = state.kreaMaskBoxes.length;
  const kind = state.kreaMaskKind === 'box' && boxCount
    ? `${boxCount} box area${boxCount === 1 ? '' : 's'} selected`
    : (state.kreaMaskKind === 'box' ? 'Box selected'
...
function renderKreaMaskMode() {
  ...
  $('#kreaMaskBoxControls').hidden = tool !== 'box';
  $('#kreaBrushField').hidden = tool !== 'brush';
  $('#kreaMaskErase').hidden = tool !== 'brush';
  $('#kreaMaskGesture').hidden = tool !== 'brush';
...
function setupMaskCanvasFromImage() {
  ...
  if (state.kreaMaskBoxes.length && state.kreaMaskKind === 'box') {
    rasterizeKreaMaskBoxes();
  } else if (state.kreaMaskPreview) {
    const im = new Image();
    im.onload = () => {
...
function normalizeKreaMaskBoxes() {
  const limit = maskBoxGeometry.MAX_BOXES;
  state.kreaMaskBoxes = (Array.isArray(state.kreaMaskBoxes) ? state.kreaMaskBoxes : [])
    .slice(0, limit)
    .map((box, index) => ({
      ...maskBoxGeometry.normalizeBox(box, index),
      color: box?.color || REGION_COLORS[index % REGION_COLORS.length],
    }));
  if (!state.kreaMaskBoxes.some((box) => box.id === state.kreaMaskActiveBoxId)) {
    state.kreaMaskActiveBoxId = state.kreaMaskBoxes[0]?.id || null;
  }
  return state.kreaMaskBoxes;
}

function activeKreaMaskBox() {
  normalizeKreaMaskBoxes();
  return state.kreaMaskBoxes.find((box) => box.id === state.kreaMaskActiveBoxId) || null;
}

function maskBoxElement(id) {
  return $$('#kreaMaskBoxes [data-mask-box-id]').find((element) => element.dataset.maskBoxId === id) || null;
}

function announceKreaMaskBox(message) {
  const live = $('#kreaMaskBoxLive');
  if (!live) return;
  live.textContent = '';
  requestAnimationFrame(() => { live.textContent = message; });
}

function renderKreaMaskBoxControls() {
  const boxes = normalizeKreaMaskBoxes();
  const activeIndex = boxes.findIndex((box) => box.id === state.kreaMaskActiveBoxId);
  $('#kreaMaskBoxAdd').disabled = boxes.length >= maskBoxGeometry.MAX_BOXES;
  $('#kreaMaskBoxDelete').disabled = activeIndex < 0;
  $('#kreaMaskBoxPrev').disabled = boxes.length < 2;
  $('#kreaMaskBoxNext').disabled = boxes.length < 2;
  $('#kreaMaskBoxCount').textContent = activeIndex < 0
    ? 'No boxes'
    : `Box ${activeIndex + 1} of ${boxes.length}`;
}

function renderKreaMaskBoxes() {
  const layer = $('#kreaMaskBoxes');
  if (!layer) return;
  const visible = state.kreaMaskTool === 'box' && !state.kreaMaskPreviewCutout;
  layer.hidden = !visible;
  renderKreaMaskBoxControls();
  if (!visible) return;
  const boxes = normalizeKreaMaskBoxes();
  const content = maskContentRect();
  if (!Number.isFinite(content.width) || !Number.isFinite(content.height) || content.width <= 0 || content.height <= 0) return;
  layer.style.left = `${content.left}px`;
  layer.style.top = `${content.top}px`;
  layer.style.width = `${content.width}px`;
  layer.style.height = `${content.height}px`;
  layer.querySelectorAll('.krea-mask-box').forEach((element) => element.remove());
  $('#kreaMaskBoxEmpty').hidden = boxes.length > 0;
  boxes.forEach((box, index) => {
    const active = box.id === state.kreaMaskActiveBoxId;
    ...
    element.style.setProperty('--mask-box-color', box.color || REGION_COLORS[index % REGION_COLORS.length]);
    const number = document.createElement('span');
    number.className = 'krea-mask-box-number';
...
function updateKreaMaskBoxElement(box) {
  const element = maskBoxElement(box.id);
  if (!element) return;
  element.style.left = `${box.x * 100}%`;
  element.style.top = `${box.y * 100}%`;
  element.style.width = `${box.w * 100}%`;
...
function discardEditableMaskBoxes() {
  if (!state.kreaMaskBoxes.length) return;
  state.kreaMaskBoxes = [];
  state.kreaMaskActiveBoxId = null;
  renderKreaMaskBoxes();
}

function rasterizeKreaMaskBoxes({ persist = false } = {}) {
  const boxes = normalizeKreaMaskBoxes();
  const canvas = $('#kreaMaskCanvas');
...
function prepareNewMaskBoxSet() {
  if (state.kreaMaskKind === 'box' && state.kreaMaskBoxes.length) return;
  state.kreaMaskBoxes = [];
  state.kreaMaskActiveBoxId = null;
}

function addKreaMaskBox() {
  prepareNewMaskBoxSet();
  if (state.kreaMaskBoxes.length >= maskBoxGeometry.MAX_BOXES) {
    toast(`Masks support up to ${maskBoxGeometry.MAX_BOXES} boxes`, true);
    return;
  }
  const index = state.kreaMaskBoxes.length;
  const box = {
    ...maskBoxGeometry.defaultBox(index, `mb${Date.now()}${Math.floor(Math.random() * 1000)}`),
    color: REGION_COLORS[index % REGION_COLORS.length],
  };
  state.kreaMaskBoxes.push(box);
  state.kreaMaskActiveBoxId = box.id;
  rasterizeKreaMaskBoxes({ persist: true });
  announceKreaMaskBox(`Added mask box ${index + 1}`);
  requestAnimationFrame(() => maskBoxElement(box.id)?.focus());
}

function deleteActiveKreaMaskBox() {
  const boxes = normalizeKreaMaskBoxes();
  const index = boxes.findIndex((box) => box.id === state.kreaMaskActiveBoxId);
  if (index < 0) return;
  boxes.splice(index, 1);
  state.kreaMaskActiveBoxId = boxes[Math.min(index, boxes.length - 1)]?.id || null;
  rasterizeKreaMaskBoxes({ persist: true });
  announceKreaMaskBox(boxes.length ? `Mask box deleted. ${boxes.length} remaining.` : 'All mask boxes removed');
}

function cycleKreaMaskBox(direction) {
  const nextId = maskBoxGeometry.cycleBoxId(state.kreaMaskBoxes, state.kreaMaskActiveBoxId, direction);
  if (!nextId) return;
  state.kreaMaskActiveBoxId = nextId;
  renderKreaMaskBoxes();
  const index = state.kreaMaskBoxes.findIndex((box) => box.id === nextId);
  announceKreaMaskBox(`Selected mask box ${index + 1} of ${state.kreaMaskBoxes.length}`);
  requestAnimationFrame(() => maskBoxElement(nextId)?.focus());
}
...
function beginKreaMaskBoxCreate(event) {
  ...
  const previous = {
    boxes: state.kreaMaskBoxes.map((box) => ({ ...box })),
    activeId: state.kreaMaskActiveBoxId,
    kind: state.kreaMaskKind,
  };
  prepareNewMaskBoxSet();
  const index = state.kreaMaskBoxes.length;
  const box = {
    ...maskBoxGeometry.normalizeBox({
      id: `mb${Date.now()}${Math.floor(Math.random() * 1000)}`,
      x: point.nx,
      y: point.ny,
      w: maskBoxGeometry.MIN_SIZE,
      h: maskBoxGeometry.MIN_SIZE,
    }, index),
    color: REGION_COLORS[index % REGION_COLORS.length],
  };
  state.kreaMaskBoxes.push(box);
...
function beginKreaMaskBoxInteraction(event) {
  ...
  $('#kreaMaskBoxes').setPointerCapture?.(event.pointerId);
}

function moveKreaMaskBoxInteraction(event) {
  const interaction = kreaMaskBoxInteraction;
  if (!interaction || interaction.pointerId !== event.pointerId) return;
  event.preventDefault();
  const box = state.kreaMaskBoxes.find((item) => item.id === interaction.boxId);
  if (!box) return;
  ...
  if (interaction.type === 'create') {
    const point = maskPoint(event);
    const rect = maskBoxGeometry.rectFromPoints(interaction.anchor, { x: point.nx, y: point.ny });
    Object.assign(box, maskBoxGeometry.normalizeBox({ ...box, ...rect }));
  } else {
    const content = maskContentRect();
    const dx = (event.clientX - interaction.startX) / content.width;
    const dy = (event.clientY - interaction.startY) / content.height;
    const next = interaction.type === 'resize'
      ? maskBoxGeometry.resizeBox(interaction.original, interaction.handle, dx, dy)
      : maskBoxGeometry.moveBox(interaction.original, dx, dy);
    Object.assign(box, next);
  }
  updateKreaMaskBoxElement(box);
}

function finishKreaMaskBoxInteraction(event, cancelled = false) {
  const interaction = kreaMaskBoxInteraction;
  if (!interaction || (event && interaction.pointerId !== event.pointerId)) return;
  kreaMaskBoxInteraction = null;
  const layer = $('#kreaMaskBoxes');
  if (layer.hasPointerCapture?.(interaction.pointerId)) layer.releasePointerCapture(interaction.pointerId);
  const box = state.kreaMaskBoxes.find((item) => item.id === interaction.boxId);
  if (cancelled || (interaction.type === 'create' && !interaction.moved)) {
    if (interaction.type === 'create') {
      state.kreaMaskBoxes = interaction.previous.boxes;
      state.kreaMaskActiveBoxId = interaction.previous.activeId;
      state.kreaMaskKind = interaction.previous.kind;
    } else if (box) Object.assign(box, interaction.original);
    renderKreaMaskBoxes();
    return;
  }
  normalizeKreaMaskBoxes();
  rasterizeKreaMaskBoxes({ persist: true });
  const active = activeKreaMaskBox();
  if (active) {
    const width = Math.round(active.w * 100);
...
function keyboardKreaMaskBox(event) {
  ...
  const next = handle
    ? maskBoxGeometry.resizeBox(box, handle, dx, dy)
    : (event.shiftKey ? maskBoxGeometry.resizeBox(box, 'se', dx, dy) : maskBoxGeometry.moveBox(box, dx, dy));
  Object.assign(box, next);
  rasterizeKreaMaskBoxes({ persist: true });
  requestAnimationFrame(() => {
    const nextElement = maskBoxElement(box.id);
    const target = handle ? nextElement?.querySelector(`[data-mask-box-handle="${handle}"]`) : nextElement;
    target?.focus();
  });
}

function drawKreaMask(e) {
  if (!kreaMaskDrawing || state.kreaMaskTool !== 'brush') return;
  e.preventDefault();
  const p = maskPoint(e);
  const canvas = $('#kreaMaskCanvas');
  const ctx = canvas.getContext('2d');
...
function previewMaskCanvas() {
  const source = processedMaskCanvas();
  if (!source || editOutpaintActive()) return source;
  const expansion = Math.max(6, Math.min(32, Math.round(Number(state.editMaskExpand) || 14)));
  const cacheKey = `${processedMaskCache.key}:expand:${expansion}`;
  if (previewMaskCache.key === cacheKey && previewMaskCache.canvas) return previewMaskCache.canvas;
  const sourceContext = source.getContext('2d', { willReadFrequently: true });
  const sourceImage = sourceContext.getImageData(0, 0, source.width, source.height);
  let values = new Uint8ClampedArray(source.width * source.height);
  for (let pixel = 0, dataIndex = 0; pixel < values.length; pixel += 1, dataIndex += 4) {
    values[pixel] = sourceImage.data[dataIndex];
  }
  values = offsetMaskValues(values, source.width, source.height, expansion, true);
  ...
  previewMaskCache = { key: cacheKey, canvas: preview };
  return preview;
}

function renderMaskOverlay() {
  const overlay = $('#kreaMaskOverlayCanvas');
  const mask = previewMaskCanvas();
...
function renderMaskAdjustments() {
  ...
  const expansion = Math.max(6, Math.min(32, Math.round(Number(state.editMaskExpand) || 14)));
  state.editMaskExpand = expansion;
  $('#kreaMaskExpandField').hidden = outpaint;
  $('#kreaMaskExpand').value = String(expansion);
  $('#kreaMaskExpandVal').textContent = `${expansion} px`;
...
function cancelSmartMaskRequest() {
  smartMaskRequestRevision += 1;
  smartMaskRunning = false;
  clearTimeout(smartMaskSlowTimer);
  smartMaskSlowTimer = null;
  const loading = $('#kreaMaskLoading');
...
async function runSmartMask({ prompt = '', point = null } = {}) {
  ...
  const requestRevision = ++smartMaskRequestRevision;
  const sourceName = ref.name;
  const requestIsCurrent = () => requestRevision === smartMaskRequestRevision
    && state.kreaMaskTool === 'smart'
    && state.refs[0]?.name === sourceName;
  smartMaskRunning = true;
  setSmartMaskLoading('Starting Smart Select…');
  $('#kreaMaskLoading').hidden = false;
  clearTimeout(smartMaskSlowTimer);
  smartMaskSlowTimer = setTimeout(() => {
    if (smartMaskRunning && requestIsCurrent()) setSmartMaskLoading('Still loading SAM3 — first use can take a few minutes');
  }, 12000);
  try {
    ...
    if (requestRevision === smartMaskRequestRevision) {
      smartMaskRunning = false;
      clearTimeout(smartMaskSlowTimer);
      smartMaskSlowTimer = null;
...
$('#kreaMaskBoxes').addEventListener('pointerup', (event) => finishKreaMaskBoxInteraction(event));
$('#kreaMaskBoxes').addEventListener('pointercancel', (event) => finishKreaMaskBoxInteraction(event, true));
$('#kreaMaskBoxes').addEventListener('lostpointercapture', (event) => finishKreaMaskBoxInteraction(event));
$('#kreaMaskBoxes').addEventListener('keydown', keyboardKreaMaskBox);
$('#kreaMaskBoxAdd').addEventListener('click', addKreaMaskBox);
$('#kreaMaskBoxDelete').addEventListener('click', deleteActiveKreaMaskBox);
...
if (typeof ResizeObserver !== 'undefined') {
  const kreaMaskStageResizeObserver = new ResizeObserver(() => {
    if (!$('#kreaMaskSheet').classList.contains('show')) return;
    renderSmartMaskPoints();
    renderKreaMaskBoxes();
  });
  kreaMaskStageResizeObserver.observe($('#kreaMaskStage'));
}
...
let loraPickPointerCandidate = null;
let loraPickSuppressClickUntil = 0;
...
function closeLoraPicker() {
  ...
  loraPickHandler = null;
  loraPickAllowNone = false;
  loraPickExcluded = new Set();
  loraPickPointerCandidate = null;
  syncSheetScrollLock();
}
function commitLoraPick(name) {
  try {
    if (loraPickHandler) {
    ...
    toast(error?.message || 'Could not add that LoRA', true);
    return false;
  }
}
function renderLoraPicker(query) {
  ...
  if (loraPickAllowNone && !q) {
    ...
    none.dataset.loraPickName = '';
    none.addEventListener('click', () => {
      if (performance.now() >= loraPickSuppressClickUntil) commitLoraPick('');
    });
  ...
  for (const name of names.slice(0, 200)) {
    ...
    row.dataset.loraPickName = name;
    row.innerHTML = `${loraThumbHtml(name, 'lp-thumb')}${escapeHtml(prettyLora(name))}`;
    row.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      if (performance.now() >= loraPickSuppressClickUntil) commitLoraPick(name);
    });
...
$('#loraPickList').addEventListener('pointerdown', (event) => {
  ...
  if (!row) return;
  loraPickPointerCandidate = {
    pointerId: event.pointerId,
    name: row.dataset.loraPickName,
    x: event.clientX,
    y: event.clientY,
  };
});
window.addEventListener('pointerup', (event) => {
  const candidate = loraPickPointerCandidate;
  if (!candidate || candidate.pointerId !== event.pointerId) return;
  loraPickPointerCandidate = null;
  if (!$('#loraPickSheet').classList.contains('show')) return;
  if (Math.hypot(event.clientX - candidate.x, event.clientY - candidate.y) > 10) return;
  loraPickSuppressClickUntil = performance.now() + 400;
  event.preventDefault();
  event.stopPropagation();
  commitLoraPick(candidate.name);
}, true);
window.addEventListener('pointercancel', (event) => {
  if (loraPickPointerCandidate?.pointerId === event.pointerId) loraPickPointerCandidate = null;
}, true);
...
let directorStoryboardInsertIndex = null;
...
function directorFramesToSeconds(frames) {
  return (Math.max(0, Number(frames) || 0) / DIRECTOR_FPS).toFixed(2).replace(/0+$/, '').replace(/\.$/, '');
}
...
function directorUsesPartialRange(project = state.directorProject) {
  ...
  return project.durationFrames > DIRECTOR_MAX_WINDOW
    || project.range.startFrame > 0
    || project.range.lengthFrames < project.durationFrames;
}
...
function directorExtensionVideo(source = state.directorProject?.extensionSource) {
  if (!source || directorExtensionIsUpload(source)) return null;
...
function directorExtensionUses4K(source = state.directorProject?.extensionSource) {
  if (directorExtensionIsUpload(source)) {
    return Math.max(Number(source.sourceWidth) || 0, Number(source.sourceHeight) || 0) > 1536
      && Number(source.sourceWidth) >= 512 && Number(source.sourceHeight) >= 512;
  }
  const found = directorExtensionVideo(source);
  const info = found?.video?.info || {};
...
function directorSeedProject() {
  ...
  return {
    ...
    output: {
      ...
      seed: seedInput !== '' && Number.isSafeInteger(seed) && seed >= 0 ? seed : '',
...
function directorNormalizeClientProject(value) {
  ...
  const cleanSegment = (entry, track, index) => {
    ...
    if (track === 'main') {
      ...
      videoFile: String(item.videoFile || item.assetName || ''), fileName: String(item.fileName || item.videoFile || 'Motion guide'),
      trimStart: Math.max(0, Math.round(Number(item.trimStart) || 0)),
  ...
  if (source.extensionSource != null) {
    const candidate = source.extensionSource && typeof source.extensionSource === 'object' ? source.extensionSource : {};
    const itemId = String(candidate.itemId || '').trim();
    const videoId = String(candidate.videoId || '').trim();
    const inputName = String(candidate.inputName || '').trim();
    if (inputName) {
      if (inputName.length > 512 || inputName.includes('..') || inputName.startsWith('/') || inputName.startsWith('\\')) {
        throw new Error('The uploaded continuation source is invalid');
      }
      extensionSource = {
        inputName,
        fileName: String(candidate.fileName || candidate.label || 'Uploaded video').slice(0, 255),
        continueAudio: candidate.continueAudio !== false,
        sourceSeconds: Math.max(0, Number(candidate.sourceSeconds) || 0),
        sourceWidth: Math.max(0, Math.round(Number(candidate.sourceWidth) || 0)),
        sourceHeight: Math.max(0, Math.round(Number(candidate.sourceHeight) || 0)),
        sourceHasAudio: candidate.sourceHasAudio === true,
      };
    } else {
      if (!/^[a-zA-Z0-9_-]{1,80}$/.test(itemId) || !/^[a-zA-Z0-9_-]{1,80}$/.test(videoId)) {
        throw new Error('The continuation source is invalid');
      }
      extensionSource = {
        itemId, videoId,
        fileName: String(candidate.fileName || candidate.label || 'Video').slice(0, 255),
        continueAudio: candidate.continueAudio !== false,
        sourceSeconds: Math.max(0, Number(candidate.sourceSeconds) || 0),
      };
    }
  ...
  return {
    ...
    output: {
      ...
      seed: output.seed !== '' && output.seed != null && Number.isSafeInteger(Number(output.seed)) && Number(output.seed) >= 0 ? Number(output.seed) : '',
...
function directorOverlapError(project = state.directorProject) {
  ...
  if (project.extensionSource) {
    ...
    if (!source && !directorExtensionIsUpload(project.extensionSource)) return 'The continuation source is no longer in this gallery.';
  ...
  if (project.range.lengthFrames > DIRECTOR_MAX_WINDOW || project.range.startFrame + project.range.lengthFrames > project.durationFrames) return 'The selected range must stay within the sequence and be no longer than 20 seconds.';
...
function setDirectorResolutionExpanded(expanded) {
  const picker = $('#directorResolutionPicker');
  const header = $('#directorResolutionHeader');
  const body = $('#directorResolutionBody');
  if (!picker || !header || !body) return;
  picker.classList.toggle('expanded', expanded);
...
function renderDirectorResolutionPicker() {
  const project = state.directorProject;
  const row = $('#directorAspectRow');
  if (!project || !row) return;
  const width = project.output.width;
  const height = project.output.height;
  const ratio = width / Math.max(1, height);
  const match = ASPECTS.reduce((best, aspect) => {
    const error = Math.abs(aspect.ar - ratio) / ratio;
    return !best || error < best.error ? { aspect, error } : best;
  }, null);
  const selectedAspect = match?.error <= 0.025 ? match.aspect : null;
  const selectedMp = nearestResolutionMegapixels(width, height);
  const actualMp = width * height / 1e6;
  const sizeMatches = Math.abs(actualMp - selectedMp) <= 0.12;
  const label = selectedAspect?.label || derivedAspectLabel(width, height);
  $('#directorResolutionSummary').textContent = `${label} · ${width} × ${height}`;
  $('#directorResolutionDimensions').textContent = `${width} × ${height}`;
  const icon = $('#directorResolutionIcon');
  const base = 14;
  icon.style.width = `${ratio >= 1 ? base : Math.max(7, Math.round(base * ratio))}px`;
  icon.style.height = `${ratio >= 1 ? Math.max(7, Math.round(base / ratio)) : base}px`;
  row.replaceChildren(...ASPECTS.map((aspect) => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'aspect-chip' + (selectedAspect?.label === aspect.label ? ' active' : '');
    const maxSide = 22;
    const iconWidth = aspect.ar >= 1 ? maxSide : Math.max(7, Math.round(maxSide * aspect.ar));
    const iconHeight = aspect.ar >= 1 ? Math.max(7, Math.round(maxSide / aspect.ar)) : maxSide;
    button.innerHTML = `<span class="ar-box" style="width:${iconWidth}px;height:${iconHeight}px"></span>${aspect.label}`;
    button.addEventListener('click', () => {
      const dimensions = dimensionsForMegapixels(aspect.ar, sizeMatches ? selectedMp : 1);
      project.output.width = dimensions.w;
      project.output.height = dimensions.h;
      renderDirector();
      saveDirectorProject();
    });
    return button;
  }));
  $$('#directorSizeSeg button').forEach((button) => {
    button.classList.toggle('active', sizeMatches && Number(button.dataset.directorMp) === selectedMp);
  });
}

function renderDirectorDurationControl() {
  const project = state.directorProject;
  const range = $('#directorDurationRange');
  const shell = $('#directorDurationSlider');
  if (!project || !range || !shell) return;
  const value = Number(directorFramesToSeconds(project.durationFrames));
  const min = project.extensionSource ? 1 : (value < 1 ? 0.04 : 1);
  const max = Math.max(20, Math.ceil(value));
  range.min = String(min);
  range.max = String(max);
  range.step = '0.1';
  range.value = String(value);
  const progress = max > min ? directorClamp((value - min) / (max - min) * 100, 0, 100) : 0;
  shell.style.setProperty('--duration-progress', `${progress}%`);
  shell.style.setProperty('--duration-visual-progress', `${progress}%`);
  $('#directorDurationBubble').textContent = formatVideoDuration(value);
  $('#directorDurationMin').textContent = `${formatVideoDuration(min)}s`;
  $('#directorDurationMax').textContent = `${formatVideoDuration(max)}s`;
  const increment = max - min > 30 ? 5 : 1;
  const tickValues = [];
  for (let tick = Math.ceil(min / increment) * increment; tick <= max; tick += increment) tickValues.push(tick);
  if (!tickValues.includes(min)) tickValues.unshift(min);
  if (!tickValues.includes(max)) tickValues.push(max);
  $('#directorDurationTicks').replaceChildren(...tickValues.map((tick) => {
    const mark = document.createElement('i');
    mark.className = 'major';
    const tickProgress = max > min ? (tick - min) / (max - min) * 100 : 0;
    if (Math.abs(tickProgress) < 0.001) mark.classList.add('edge-start');
    if (Math.abs(tickProgress - 100) < 0.001) mark.classList.add('edge-end');
...
function directorSetFormValues() {
  ...
  $('#directorDuration').value = directorFramesToSeconds(project.durationFrames);
  ...
  renderDirectorResolutionPicker();
  renderDirectorDurationControl();
  $$('#directorSmoothRow [data-director-smooth]').forEach((button) => {
    button.classList.toggle('active', Number(button.dataset.directorSmooth) === project.output.smooth);
  });
  ...
  $('#directorDuration').min = extending ? '1' : '0.04';
  for (const id of ['directorResolutionHeader', 'directorFourK']) {
    const control = $(`#${id}`);
    if (control) control.disabled = extending;
  }
}
...
function openDirectorLoraPicker() {
  const selected = directorLoras();
  openLoraPicker((name) => {
    const loras = directorLoras();
    if (!name || loras.some((lora) => lora.name === name)) return;
    const lora = { name, strength: 1, on: true };
    applyContextLoraDefault(lora);
    state.directorProject.output.loras = [...loras, lora];
    renderDirectorLoras();
    saveDirectorProject();
    toast(`${prettyLora(name)} added to Director`);
  }, { title: 'Add Director LoRA', exclude: selected.map((lora) => lora.name) });
}
...
function directorSegmentLabel(segment) {
  ...
  return [segment.isStaticImage ? 'Motion guide image' : 'Motion guide video', segment.fileName || segment.videoFile];
}
...
function directorKeyframes(project = state.directorProject) {
  return (project?.segments || []).filter((segment) => segment.type === 'image').sort((a, b) => a.start - b.start);
}
...
function directorRedistributeStoryItems(ordered = directorStoryItems()) {
  const project = state.directorProject;
  if (!project || !ordered.length) return;
  const previousDuration = project.durationFrames;
...
function directorRedistributeKeyframes(ordered = directorKeyframes()) {
  const story = directorStoryItems();
  let imageIndex = 0;
  const merged = story.map((segment) => segment.type === 'image' ? ordered[imageIndex++] : segment).filter(Boolean);
  merged.push(...ordered.slice(imageIndex));
  directorRedistributeStoryItems(merged);
}

function directorMoveKeyframe(id, direction) {
...
function directorRemoveKeyframe(id) {
  const project = state.directorProject;
  if (!project) return;
...
function directorRefreshKeyframeDomOrder(list) {
  [...list.querySelectorAll('[data-director-story-id]')].forEach((card, index, cards) => {
    ...
    if (time) time.textContent = index === 0 ? 'Starts the story'
      : (index === cards.length - 1 ? 'Ends the story' : `Story beat ${index + 1}`);
  });
}

function directorCommitKeyframeDomOrder(list) {
  const byId = new Map(directorStoryItems().map((segment) => [segment.id, segment]));
  const ordered = [...list.querySelectorAll('[data-director-story-id]')]
    .map((card) => byId.get(card.dataset.directorStoryId))
    .filter(Boolean);
...
function wireDirectorKeyframeDrag(card, list) {
  ...
  const move = (event) => {
    ...
    event.preventDefault();
    const siblings = [...list.querySelectorAll('[data-director-story-id]')].filter((entry) => entry !== card);
  ...
  card.addEventListener('pointerdown', (event) => {
    if (event.target.closest('button,input,textarea') || (event.pointerType === 'mouse' && event.button !== 0)) return;
...
function renderDirectorKeyframePreset() {
  ...
  story.forEach((segment, index) => {
    ...
    if (segment.type === 'image') {
      ...
      media.appendChild(directionIcon);
    }
    ...
    if (segment.type === 'text') {
      ...
      prompt.rows = 2;
      prompt.placeholder = 'Describe what happens here…';
      prompt.setAttribute('aria-label', `Direction for story beat ${index + 1}`);
      prompt.addEventListener('input', () => {
        segment.prompt = prompt.value.slice(0, 4000);
        directorRefreshGenerationValidation();
        saveDirectorProject();
      });
      copy.append(prompt);
    }
    copy.append(time);
    const actions = document.createElement('span');
    actions.className = 'director-keyframe-actions';
    for (const [label, delta, disabled] of [['Move earlier', -1, index === 0], ['Move later', 1, index === story.length - 1]]) {
      const button = document.createElement('button');
      button.type = 'button';
      button.disabled = disabled;
      button.setAttribute('aria-label', `${label}: ${title.textContent}`);
      button.textContent = delta < 0 ? '‹' : '›';
      button.addEventListener('click', () => directorMoveKeyframe(segment.id, delta));
    ...
    remove.textContent = '×';
    remove.addEventListener('click', () => directorRemoveKeyframe(segment.id));
    actions.appendChild(remove);
    card.append(media, copy, actions);
    list.appendChild(card);
    wireDirectorKeyframeDrag(card, list);
  });
  appendAdd(story.length);
  list.classList.toggle('is-empty', story.length === 0);
  if (!story.length) {
...
function renderDirectorPreset() {
  const mode = state.directorComposerMode;
  const project = state.directorProject;
  ...
  const streamlined = mode !== 'timeline';
  preset.hidden = !streamlined;
  timeline.hidden = streamlined;
  $('#directorExtendPreset').hidden = mode !== 'extend';
...
function renderDirectorSelectionControls() {
  ...
  if (!controls || !selection || !segment || state.directorComposerMode !== 'timeline') {
    if (controls) controls.hidden = true;
    return;
  }
  const row = $(`#director${selection.track === 'main' ? 'Story' : (selection.track === 'audio' ? 'Audio' : 'Motion')}TrackRow`);
  if (!row || row.hidden) {
...
function renderDirectorRuler(ppf) {
  ...
  const seconds = Math.floor(state.directorProject.durationFrames / DIRECTOR_FPS);
...
function directorOpenSheet(id) {
  const sheet = typeof id === 'string' ? document.querySelector(id.startsWith('#') ? id : `#${id}`) : id;
  if (!sheet) return;
  sheet.classList.add('show');
  syncSheetScrollLock();
}

function directorCloseSheet(id) {
  const sheet = typeof id === 'string' ? document.querySelector(id.startsWith('#') ? id : `#${id}`) : id;
  if (!sheet) return;
  sheet.classList.remove('show');
  syncSheetScrollLock();
...
function directorOpenInspector() {
  if (!directorSelected()) return;
  const inspector = $('#directorInspector');
  if (!inspector) return;
...
function directorCloseInspector() {
  const inspector = $('#directorInspector');
  const layoutChanged = inspector?.classList.contains('show') === true;
  if (inspector) {
...
function renderDirectorInspector() {
  const segment = directorSelected();
  if ($('#directorEmptyInspector')) $('#directorEmptyInspector').hidden = !!segment;
  if ($('#directorSegmentInspector')) $('#directorSegmentInspector').hidden = !segment;
  if (!segment) {
    directorCloseInspector();
    return;
  }
  ...
  if ($('#directorSegmentStartSeconds')) $('#directorSegmentStartSeconds').value = directorFramesToSeconds(segment.start);
  if ($('#directorSegmentDurationSeconds')) $('#directorSegmentDurationSeconds').value = directorFramesToSeconds(segment.length);
  ...
  const nameField = $('#directorSegmentNameField');
  if (nameField) nameField.hidden = segment.type === 'text';
  if ($('#directorSegmentName')) $('#directorSegmentName').value = segment.fileName || directorAssetName(segment);
...
function renderDirector() {
  ...
  const extensionSeconds = extension ? directorExtensionSeconds(extension) : 0;
  const extensionBanner = $('#directorExtensionSource');
  if (extensionBanner) {
    extensionBanner.hidden = !extension;
    if (extension) {
      $('#directorExtensionSourceName').textContent = extension.fileName || extensionEntry?.video?.file || 'Source video';
  ...
  const timelineEmpty = project.segments.length === 0 && project.audioSegments.length === 0 && project.motionSegments.length === 0;
  $('#directorTimelineViewport').classList.toggle('is-empty', timelineEmpty);
  $('#directorTimelineViewport').tabIndex = timelineEmpty ? -1 : 0;
  $('#directorAddBtn').textContent = timelineEmpty ? '＋ Add to timeline' : '+ Add';
  $('#directorAddBtn').setAttribute('aria-label', timelineEmpty ? 'Add the first item to the timeline' : 'Add to Story');
  ...
  if ($('#directorSummaryLabel')) $('#directorSummaryLabel').textContent = extension ? 'Extension' : (state.directorComposerMode === 'keyframes' ? 'Storyboard video' : (partialRange ? 'Selected range' : 'Sequence'));
  $('#directorSummary').textContent = extension
    ? `${directorFramesToSeconds(project.range.lengthFrames)}s new · ${(extensionSeconds + newSeconds).toFixed(1)}s total · ${project.output.width}×${project.output.height}${project.output.fourK ? ' · 4K' : ''}${project.output.batch > 1 ? ` · ${project.output.batch} videos` : ''}`
    : `${directorFramesToSeconds(project.range.lengthFrames)}s · ${project.output.width}×${project.output.height}${project.output.fourK ? ' · 4K' : ''}${project.output.batch > 1 ? ` · ${project.output.batch} videos` : ''}`;
  const error = directorOverlapError(project);
  const directionError = error === 'Add an overall or timed direction.' ? error : '';
  const selected = directorSelected();
  const selectedAsset = directorAssetName(selected || {});
  const selectedMediaError = selectedAsset && directorMissingAssets.has(selectedAsset)
    ? 'This media is missing. Replace or delete it.'
    : '';
  const inspectorVisible = $('#directorInspector')?.classList.contains('show') === true;
  if ($('#directorPromptValidation')) $('#directorPromptValidation').textContent = directionError;
  ...
  if (directorNeedsRangeScroll) {
    ...
    requestAnimationFrame(() => {
      const viewport = $('#directorTimelineViewport');
      if (!viewport) return;
      viewport.scrollLeft = Math.max(0, project.range.startFrame * ppf - 16);
    });
  }
}
...
function startDirectorWorkflow(mode) {
  ...
  const project = directorSeedProject();
  project.extensionSource = null;
  project.audioSegments = [];
  project.motionSegments = [];
  if (mode === 'extend') project.segments = [];
  openDirectorMode(project, { mode });
  if (mode === 'keyframes') directorRedistributeKeyframes();
  else requestAnimationFrame(directorChooseExtensionSource);
}

function switchDirectorWorkflow(mode) {
  if (!['extend', 'keyframes', 'timeline'].includes(mode) || !state.directorProject) return;
  if (mode === 'keyframes' && state.directorProject.extensionSource) {
    const project = directorSeedProject();
    project.extensionSource = null;
    project.audioSegments = [];
    project.motionSegments = [];
  ...
  renderDirector();
  saveDirectorProject();
  if (mode === 'extend' && !state.directorProject.extensionSource) directorChooseExtensionSource();
}

function directorProbeVideo(url) {
  return new Promise((resolve) => {
    const video = document.createElement('video');
    video.preload = 'metadata';
    video.muted = true;
    video.onloadedmetadata = () => resolve({
      seconds: Math.max(0, Number(video.duration) || 0),
...
async function openDirectorUploadedExtension(asset) {
  const metadata = await directorProbeVideo(asset.url || `/api/input?name=${encodeURIComponent(asset.name)}`);
  if (!(metadata.seconds > 0) || !(metadata.width > 0) || !(metadata.height > 0)) {
    toast('Mix Studio could not read that video. Try an MP4, WebM, or MOV file.', true);
    return;
  }
  const fourK = Math.max(metadata.width, metadata.height) > 1536 && metadata.width >= 512 && metadata.height >= 512;
  const limit = fourK ? 10 : 20;
  if (metadata.seconds > limit + 0.08) {
    toast(`Video extension supports source clips up to ${limit} seconds${fourK ? ' at 4K' : ''}.`, true);
    return;
  }
  const project = directorSeedProject();
  project.durationFrames = DIRECTOR_FPS * 5;
  project.range = { startFrame: 0, lengthFrames: project.durationFrames };
  project.globalPrompt = promptDraft().trim() || 'Continue the action naturally from the final frame.';
  project.segments = [];
  project.audioSegments = [];
  project.motionSegments = [];
  project.extensionSource = {
    inputName: asset.name,
    fileName: asset.label || 'Uploaded video',
    sourceSeconds: metadata.seconds,
  ...
  project.output.width = Math.max(256, Math.round((metadata.width / scale) / 64) * 64);
  project.output.height = Math.max(256, Math.round((metadata.height / scale) / 64) * 64);
  project.output.fourK = fourK;
  openDirectorMode(project, { mode: 'extend' });
  toast('Extension ready — describe what should happen next.');
}

function directorChooseExtensionSource() {
  pickUpload('video/*', async (asset) => {
    if (asset.srcItemId && asset.srcVideoId) {
      const item = state.items.find((entry) => entry.id === asset.srcItemId);
      const video = item?.videos?.find((entry) => entry.id === asset.srcVideoId);
      if (!item || !video) return toast('That gallery video is no longer available.', true);
      openDirectorExtension(item, video);
      return;
    }
    await openDirectorUploadedExtension(asset);
...
function openDirectorExtension(item, video) {
  ...
  project.durationFrames = DIRECTOR_FPS * 5;
  project.range = { startFrame: 0, lengthFrames: project.durationFrames };
  project.globalPrompt = info.motionPrompt || item.refinedPrompt || item.prompt || 'Continue the action naturally from the final frame.';
  project.segments = [];
  project.audioSegments = [];
  project.motionSegments = [];
  ...
  project.output.fourK = fourK;
  project.output.smooth = fps >= 60 ? 3 : (fps >= 40 ? 2 : 1);
  const videoIndex = Math.max(0, (item.videos || []).findIndex((candidate) => candidate.id === video.id));
  project.extensionSource = {
    itemId: item.id,
    videoId: video.id,
    fileName: item.name ? `${item.name} · Video ${videoIndex + 1}` : `Video ${videoIndex + 1}`,
    sourceSeconds,
    continueAudio: true,
  };
  openDirectorMode(project, { mode: 'extend' });
  toast('Extension ready — direct the new footage, then generate the joined video.');
}
...
function directorInsertRelative(track, anchor, side, length) {
  const project = state.directorProject;
  const insertionFrame = side === 'before' ? anchor.start : anchor.start + anchor.length;
  const shifted = directorTrackSegments(track).filter((segment) => segment.start >= insertionFrame);
  const requiredEnd = Math.max(
    insertionFrame + length,
    ...directorTrackSegments(track).map((segment) => segment.start + segment.length + (segment.start >= insertionFrame ? length : 0)),
  );
  if (requiredEnd > DIRECTOR_MAX_FRAMES) return -1;
  const previousDuration = project.durationFrames;
  const wasWholeSequence = project.range.startFrame === 0 && project.range.lengthFrames === previousDuration;
  shifted.forEach((segment) => { segment.start += length; });
  project.durationFrames = Math.max(previousDuration, requiredEnd);
  if (wasWholeSequence) {
    project.range.startFrame = 0;
    project.range.lengthFrames = Math.min(DIRECTOR_MAX_WINDOW, project.durationFrames);
  } else if (insertionFrame <= project.range.startFrame) {
    project.range.startFrame = Math.min(project.durationFrames - project.range.lengthFrames, project.range.startFrame + length);
  } else if (insertionFrame < project.range.startFrame + project.range.lengthFrames) {
    project.range.lengthFrames = Math.min(DIRECTOR_MAX_WINDOW, project.range.lengthFrames + length, project.durationFrames - project.range.startFrame);
  }
...
function directorOpenAddRelative(side) {
  ...
  directorConfigureAddSheet(false);
  directorStoryboardInsertIndex = null;
...
function directorOpenStoryboardAdd(index) {
  const count = directorStoryItems().length;
  directorStoryboardInsertIndex = Math.round(directorClamp(index, 0, count));
  directorAddContext = null;
  directorReplaceTarget = null;
  directorConfigureAddSheet(true);
  $('#directorAddTitle span').textContent = count ? 'Add a story beat' : 'Start the storyboard';
  directorOpenSheet('directorAddSheet');
}
...
function directorApplyTiming() {
  ...
  const start = Math.round(directorClamp(Number($('#directorSegmentStart').value), 0, state.directorProject.durationFrames - length));
  if (directorTimingOverlaps(track, start, length, segment.id)) {
    toast('That timing overlaps another segment', true);
    renderDirector();
    directorOpenInspector();
    if ($('#directorInspectorValidation')) $('#directorInspectorValidation').textContent = 'That timing overlaps another segment.';
    return;
  }
  segment.start = start;
...
function directorApplySecondsTiming() {
  const segment = directorSelected();
  if (!segment) return;
  const track = directorTrackFor(segment);
  const length = Math.min(
    state.directorProject.durationFrames,
    directorSecondsToFrames($('#directorSegmentDurationSeconds').value, { minimum: 1 }),
  );
  const start = Math.round(directorClamp(
    directorSecondsToFrames($('#directorSegmentStartSeconds').value),
    0,
    state.directorProject.durationFrames - length,
  ));
  if (directorTimingOverlaps(track, start, length, segment.id)) {
    toast('That timing overlaps another segment', true);
    renderDirector();
...
async function directorHandlePickedMedia(asset, kind) {
  ...
  try {
    if (kind === 'image' && state.directorComposerMode === 'keyframes' && !directorReplaceTarget) {
      const ordered = directorStoryItems();
      const available = Math.max(0, DIRECTOR_TRACK_LIMIT - ordered.length);
      const accepted = assets.slice(0, available);
      if (!accepted.length) return toast('This storyboard already has 256 items', true);
      ...
      directorStoryboardInsertIndex = null;
      directorAddContext = null;
      ordered.splice(insertion, 0, ...segments);
      directorRedistributeStoryItems(ordered);
      state.directorSelection = null;
      renderDirector();
      saveDirectorProject();
...
$('#directorExtensionSourceRemove').addEventListener('click', () => {
  ...
  renderDirector();
  saveDirectorProject();
  toast('Continuation source removed');
});
...
$('#directorAddPrompt').addEventListener('click', () => {
  directorCloseSheet('directorAddSheet');
  if (state.directorComposerMode === 'keyframes' && directorStoryboardInsertIndex != null) {
    const ordered = directorStoryItems();
    const insertion = Math.round(directorClamp(directorStoryboardInsertIndex, 0, ordered.length));
    const segment = { id: directorId('prompt'), type: 'text', start: 0, length: 1, prompt: '' };
    directorStoryboardInsertIndex = null;
    ordered.splice(insertion, 0, segment);
    directorRedistributeStoryItems(ordered);
    state.directorSelection = null;
...
$('#directorSegmentStartSeconds').addEventListener('change', directorApplySecondsTiming);
$('#directorSegmentDurationSeconds').addEventListener('change', directorApplySecondsTiming);
...
$('#directorSegmentName').addEventListener('input', () => { const segment = directorSelected(); if (segment) { segment.fileName = $('#directorSegmentName').value.slice(0, 240); renderDirector(); saveDirectorProject(); } });
...
for (const selector of ['#directorDuration', '#directorRangeStart', '#directorRangeLength', '#directorBatch', '#directorSeed']) {
  $(selector).addEventListener('change', directorUpdateProjectFields);
}
$('#directorDurationRange').addEventListener('input', () => {
  $('#directorDuration').value = formatVideoDuration($('#directorDurationRange').value);
  directorUpdateProjectFields();
});
$('#directorResolutionHeader').addEventListener('click', () => {
  if ($('#directorResolutionHeader').disabled) return;
  setDirectorResolutionExpanded(!$('#directorResolutionPicker').classList.contains('expanded'));
});
$('#directorSizeSeg').addEventListener('click', (event) => {
  const button = event.target.closest('[data-director-mp]');
  if (!button || !state.directorProject || state.directorProject.extensionSource) return;
  const ratio = state.directorProject.output.width / Math.max(1, state.directorProject.output.height);
  const dimensions = dimensionsForMegapixels(ratio, Number(button.dataset.directorMp));
  state.directorProject.output.width = dimensions.w;
  state.directorProject.output.height = dimensions.h;
  renderDirector();
  saveDirectorProject();
});
$('#directorSmoothRow').addEventListener('click', (event) => {
  const button = event.target.closest('[data-director-smooth]');
  if (!button || !state.directorProject) return;
  state.directorProject.output.smooth = Number(button.dataset.directorSmooth);
  renderDirector();
  saveDirectorProject();
});
...
$('#directorImportBtn').addEventListener('click', () => { directorCloseSheet('directorProjectMenuSheet'); $('#directorImportFile').click(); });
...
function setSetupCustomExpanded(expanded) {
  $('#setupCustomToggle').setAttribute('aria-expanded', String(expanded));
...
function setSetupStep(step, options = {}) {
  ...
  $$('[data-setup-tab]').forEach((tab) => {
    const active = tab.dataset.setupTab === step;
    tab.classList.toggle('active', active);
    tab.setAttribute('aria-selected', String(active));
    tab.tabIndex = active ? 0 : -1;
  });
  $$('[data-setup-pane]').forEach((pane) => {
    const active = pane.dataset.setupPane === step;
    pane.classList.toggle('active', active);
    pane.hidden = !active;
  });
...
function renderInitialSetup() {
  ...
  $('#setupTabInstall').classList.toggle('working', dependencyBusy);
  $('#setupTabFinish').classList.toggle('complete', workflowReady);
...
$$('[data-setup-tab]').forEach((tab) => {
  ...
  tab.addEventListener('keydown', (event) => {
    const current = SETUP_STEPS.indexOf(tab.dataset.setupTab);
    let next = current;
    if (event.key === 'ArrowRight') next = (current + 1) % SETUP_STEPS.length;
    else if (event.key === 'ArrowLeft') next = (current - 1 + SETUP_STEPS.length) % SETUP_STEPS.length;
    else if (event.key === 'Home') next = 0;
    else if (event.key === 'End') next = SETUP_STEPS.length - 1;
    else return;
    event.preventDefault();
    setSetupStep(SETUP_STEPS[next], { user: true, focus: true });
  });
});
...
$('#setupCancel').addEventListener('click', async () => {
  ...
  const dependencyState = setupDependencyState?.state;
  try {
    if (officialState === 'running') {
      const result = await api('/api/setup/comfy/cancel', { method: 'POST' });
      if (setupViewStatus?.comfy) setupViewStatus.comfy.install = result.install;
    } else if (dependencyState === 'running') {
