...
- **ComfyUI saves outputs forever** in its own output dir — that's the disaster-recovery source of truth. PNGs embed the full generation graph in tEXt chunks (see `recover_nathan.js` for the parser).
