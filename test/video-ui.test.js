...
test('Duration is the first video control and Motion Freedom is a separate setting', () => {
  ...
  assert.match(html, /id="vidDur"[^>]*aria-label="Video duration in seconds"[^>]*aria-describedby="vidDurationHint"/);
  ...
  assert.match(css, /\.duration-slider-progress \{[\s\S]*var\(--duration-progress\)[\s\S]*linear-gradient/);
  ...
  assert.match(app, /function videoDurationTickIncrement\(min, max\)[\s\S]*span > 30\) return 5;[\s\S]*span > 15\) return 2;/);
  assert.match(app, /style\.setProperty\('--duration-progress', `\$\{progress\}%`\)/);
