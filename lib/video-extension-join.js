...
function videoPreviewTranscodeArgs({ sourcePath, outputPath } = {}) {
  ...
  return [
    ...
    '-an',
    '-t', '5',
    '-vf', 'scale=480:480:force_original_aspect_ratio=decrease,scale=trunc(iw/2)*2:trunc(ih/2)*2,fps=12,format=yuv420p',
    '-c:v', 'libx264',
    '-preset', 'veryfast',
    '-crf', '28',
    '-g', '24',
...
async function transcodeVideoPreview({ sourcePath, outputPath, ffmpegPath } = {}, options = {}) {
  const executable = String(ffmpegPath || '').trim();
  if (!executable) {
  ...
  const run = options.run || execFileCommand;
  try {
    await run(executable, videoPreviewTranscodeArgs({ sourcePath: source, outputPath: output }), {
      cwd: path.dirname(output),
