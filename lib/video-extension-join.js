...
function mp4TranscodeArgs({ sourcePath, outputPath, fps } = {}) {
  ...
  const outputFps = filterNumber(Number.isFinite(requestedFps) && requestedFps > 0
  ...
  return [
    ...
    '-nostdin',
    '-y',
    // MediaRecorder MP4s are commonly fragmented and may omit or offset
    // presentation timestamps. Rebuild them instead of carrying browser
    // container timing into the editor-facing file.
    '-fflags', '+genpts',
    '-i', source,
    '-map', '0:v:0',
    '-map', '0:a:0?',
    '-map_metadata', '-1',
    '-map_chapters', '-1',
    '-vf', `setpts=PTS-STARTPTS,scale=trunc(iw/2)*2:trunc(ih/2)*2,setsar=1,fps=${outputFps},format=yuv420p`,
    '-af', `aresample=${AUDIO_RATE}:async=1:first_pts=0,asetpts=PTS-STARTPTS`,
    '-c:v', 'libx264',
    '-preset', 'medium',
    '-crf', '18',
    '-tag:v', 'avc1',
...
async function transcodeVideoFileToMp4({ sourcePath, outputPath, ffmpegPath, fps } = {}, options = {}) {
  const executable = String(ffmpegPath || '').trim();
  if (!executable) {
  ...
  const run = options.run || execFileCommand;
  try {
    await run(executable, mp4TranscodeArgs({ sourcePath: source, outputPath: output, fps }), {
