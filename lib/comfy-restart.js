...
function startStatus(runtime, options = {}) {
  ...
  const pathApi = options.pathApi || path;
  // Linux/DGX Spark: explicitní příkaz pro start (např. `systemctl --user start comfyui.service`).
  const startCommand = String((options.env || process.env).MIXBOX_COMFY_START_CMD || '').trim();
  if (startCommand) {
    return {
      canStart: true,
      kind: 'command',
...
async function startComfy(runtime, report = () => {}, options = {}) {
  ...
  // Linux/DGX Spark command hook: spustit sh -c "<cmd>" (např. systemctl --user start comfyui.service).
  if (status.kind === 'command' && status.command) {
    const runCommand = options.run || run;
    report('opening', 'Starting the ComfyUI service…');
    await runCommand('/bin/sh', ['-c', status.command], { timeout: 60_000 });
...
function restartStatus(runtime, options = {}) {
  const existsSync = options.existsSync || fs.existsSync;
  const platform = options.platform || process.platform;
  // Linux/DGX Spark: explicitní příkaz pro restart (např. `systemctl --user restart comfyui.service`).
  // Obchází Windows-only netstat/taskkill cestu níže.
  const restartCommand = String((options.env || process.env).MIXBOX_COMFY_RESTART_CMD || '').trim();
  if (restartCommand) {
    return {
      canRestart: true,
      kind: 'command',
...
async function restartComfy(runtime, report = () => {}, options = {}) {
  ...
  const runCommand = options.run || run;
  // Linux/DGX Spark command hook: spustit sh -c "<cmd>" (např. systemctl --user restart comfyui.service).
  if (status.kind === 'command' && status.command) {
