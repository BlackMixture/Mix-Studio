...
function findComfyDesktopApp(options = {}) {
  ...
  const candidates = [
    ...
    env.LOCALAPPDATA ? pathApi.join(env.LOCALAPPDATA, 'Programs', 'ComfyUI', 'Comfy Desktop', 'Comfy Desktop.exe') : '',
    env.ProgramFiles ? pathApi.join(env.ProgramFiles, 'ComfyUI', 'Comfy Desktop', 'Comfy Desktop.exe') : '',
...
async function launchComfyDesktop(status, options = {}) {
  if (status.desktopApp) {
    const child = (options.spawnProcess || spawn)(status.desktopApp, [], {
      cwd: path.dirname(status.desktopApp), detached: true, windowsHide: false, stdio: 'ignore',
    });
    // A failed spawn must reach the caller, not become an unhandled event
    // while the UI claims that Desktop opened successfully.
  ...
  const env = options.env || process.env;
  const powershell = path.join(String(env.SystemRoot || 'C:\\Windows'), 'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe');
  await (options.run || run)(powershell, ['-NoProfile', '-Command',
    "$ErrorActionPreference = 'Stop'; $app = Get-StartApps | Where-Object { $_.Name -match '^Comfy (Desktop|UI)$' } | Select-Object -First 1; if (-not $app) { throw 'Comfy Desktop was not found in the Start menu.' }; Start-Process ('shell:AppsFolder\\' + $app.AppID) -ErrorAction Stop",
  ], { timeout: 15000 });
}
...
async function startComfy(runtime, report = () => {}, options = {}) {
  ...
  if (options.spawn) {
    ...
    await launchComfyDesktop(status, options);
