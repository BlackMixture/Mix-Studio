...
function strengthValues({ includeZero = false, max = HUNT_MAX } = {}) {
  const ceiling = Math.max(0, Math.min(HUNT_MAX, Number.isFinite(Number(max)) ? Number(max) : HUNT_MAX));
  const values = [];
  if (includeZero) values.push(0);
  for (let value = HUNT_STEP; value < ceiling - 0.001; value += HUNT_STEP) {
    values.push(roundStrength(value));
  }
  if (ceiling > 0 && Math.abs((values[values.length - 1] ?? -1) - ceiling) > 0.001) {
    values.push(roundStrength(ceiling));
  }
  return values;
}
...
function buildStrengthHuntPlan(loras, options = {}) {
  ...
  return {
    ...
    axes: axes.map((lora, index) => ({
      ...
      maxStrength: roundStrength(lora.strength),
