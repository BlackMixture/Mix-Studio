...
function formatStrength(value) {
  const strength = roundStrength(value);
  return Number.isInteger(strength * 10) ? strength.toFixed(1) : strength.toFixed(2);
}

function strengthValues({ includeZero = false, max = HUNT_MAX } = {}) {
  const requested = Number.isFinite(Number(max)) ? Number(max) : HUNT_MAX;
  const direction = requested < 0 ? -1 : 1;
  const ceiling = Math.min(HUNT_MAX, Math.abs(requested));
  const values = [];
  if (includeZero) values.push(0);
  for (let value = HUNT_STEP; value < ceiling - 0.001; value += HUNT_STEP) {
    values.push(direction * roundStrength(value));
  }
  const endpoint = direction * ceiling;
  if (ceiling > 0 && Math.abs((values[values.length - 1] ?? -1) - endpoint) > 0.001) {
    values.push(direction * roundStrength(ceiling));
  }
  return values;
}
...
function buildStrengthHuntPlan(loras, options = {}) {
  ...
  const axisValues = axes.map((axis) => strengthValues({ includeZero: true, max: axis.strength }));
  const variants = [];
  if (axes.length === 1) {
    for (const strength of axisValues[0]) {
      variants.push({
        index: variants.length,
        row: 0,
        column: variants.length,
        strengths: [strength],
        label: `${loraDisplayName(axes[0].name)} ${formatStrength(strength)}`,
        loras: applyVariantStrengths(loras, axes, [strength]),
      });
    }
  } else {
    axisValues[1].forEach((rowStrength, row) => {
      axisValues[0].forEach((columnStrength, column) => {
        variants.push({
          index: variants.length,
          row,
          column,
          strengths: [columnStrength, rowStrength],
          label: `${loraDisplayName(axes[0].name)} ${formatStrength(columnStrength)} / ${loraDisplayName(axes[1].name)} ${formatStrength(rowStrength)}`,
  ...
  return {
    id,
    axes: axes.map((lora, index) => ({
      name: lora.name,
      label: loraDisplayName(lora.name),
      maxStrength: axisValues[index][axisValues[index].length - 1] || 0,
      values: axisValues[index],
    })),
    columns: axisValues[0].length,
    rows: axes.length === 1 ? 1 : axisValues[1].length,
    variants,
  };
}
...
function drawScaledImage(canvas, canvasWidth, canvasHeight, image, x, y, width, height) {
  ...
  for (let py = 0; py < outHeight; py += 1) {
    const sourceY = Math.max(0, Math.min(image.height - 1, (py + 0.5) / scale - 0.5));
    const y0 = Math.floor(sourceY);
    const y1 = Math.min(image.height - 1, y0 + 1);
    const fy = sourceY - y0;
    for (let px = 0; px < outWidth; px += 1) {
      const sourceX = Math.max(0, Math.min(image.width - 1, (px + 0.5) / scale - 0.5));
      ...
      for (let channel = 0; channel < 4; channel += 1) {
        const topValue = image.pixels[topLeft + channel] * (1 - fx) + image.pixels[topRight + channel] * fx;
        const bottomValue = image.pixels[bottomLeft + channel] * (1 - fx) + image.pixels[bottomRight + channel] * fx;
        canvas[target + channel] = Math.round(topValue * (1 - fy) + bottomValue * fy);
      }
    }
  }
}

function strengthHuntSheetLayout(columns, rows, ratio, options = {}) {
  columns = Math.max(1, Number(columns) || 1);
  rows = Math.max(1, Number(rows) || 1);
  ratio = Math.max(0.01, Number(ratio) || 1);
  const matrix = rows > 1;
  const labelHeight = matrix ? 34 : 38;
  const gap = matrix ? 8 : 10;
  const margin = 24;
  const headerHeight = 150;
  // Keep matrices shareable and memory-safe while giving each comparison
  // substantially more pixels than the original 180px thumbnail sheet.
  const maxWidth = Number(options.maxWidth) || (matrix ? 4096 : 6144);
  const maxHeight = Number(options.maxHeight) || 4096;
  const desiredTileWidth = Number(options.desiredTileWidth) || (matrix ? 360 : 512);
  const widthLimited = Math.floor((maxWidth - margin * 2 - Math.max(0, columns - 1) * gap) / columns);
  const rowBudget = Math.floor((maxHeight - headerHeight - margin * 2 - Math.max(0, rows - 1) * gap) / rows);
  const heightLimited = Math.floor(Math.max(64, rowBudget - labelHeight) / Math.max(0.01, ratio));
  const tileWidth = Math.max(64, Math.min(desiredTileWidth, widthLimited, heightLimited));
  const imageHeight = Math.max(64, Math.round(tileWidth * ratio));
  ...
  return { columns, rows, matrix, tileWidth, imageHeight, labelHeight, gap, margin, headerHeight, width, height };
}
...
function buildStrengthHuntSheet(images, info = {}) {
  ...
  const layout = strengthHuntSheetLayout(columns, rows, decodedFirst.height / decodedFirst.width, {
    desiredTileWidth: info.desiredTileWidth,
  });
  const {
    matrix, tileWidth, imageHeight, labelHeight, gap, margin, headerHeight,
    width: contentWidth, height: contentHeight,
  } = layout;
  const squareCanvas = info.squareCanvas === true;
  const width = squareCanvas ? Math.max(contentWidth, contentHeight) : contentWidth;
  const height = squareCanvas ? width : contentHeight;
  const originX = Math.floor((width - contentWidth) / 2);
  const originY = Math.floor((height - contentHeight) / 2);
  const pixels = Buffer.alloc(width * height * 4);
  fill(pixels, width, height, [10, 11, 14, 255]);
  drawText(pixels, width, height, 'LORA STRENGTH HUNT', originX + margin, originY + 22, { scale: 3, color: [255, 255, 255, 255] });
  const settings = `SEED ${info.seed ?? '-'}  CFG ${info.cfg ?? '-'}  STEPS ${info.steps ?? '-'}  ${info.model || ''}`;
  drawText(pixels, width, height, settings, originX + margin, originY + 54, { scale: 2, color: [171, 184, 210, 255] });
  const axes = Array.isArray(info.axes) ? info.axes : [];
  const axisName = (axis) => {
    const maximum = Number(axis && axis.maxStrength);
    return `${axis && (axis.label || axis.name) || 'LORA'}${Number.isFinite(maximum) ? ` (MAX ${formatStrength(maximum)})` : ''}`;
  };
  const axisText = axes.length > 1
    ? `X: ${axisName(axes[0])}  Y: ${axisName(axes[1])}`
    : (axes.length ? `LORA: ${axisName(axes[0])}` : '');
  if (axisText) drawText(pixels, width, height, axisText, originX + margin, originY + 76, { scale: 1, color: [139, 177, 255, 255] });
  wrapText(`PROMPT: ${info.prompt || ''}`, Math.max(24, Math.floor((contentWidth - margin * 2) / 12)), 3)
    .forEach((line, index) => drawText(pixels, width, height, line, originX + margin, originY + 92 + index * 17, { scale: 2, color: [222, 225, 232, 255] }));
  list.forEach((entry, index) => {
    ...
    const x = originX + margin + column * (tileWidth + gap);
    const y = originY + headerHeight + margin + row * (labelHeight + imageHeight + gap);
    ...
    const strengthLabel = strengths.length > 1
      ? `X ${formatStrength(strengths[0])} / Y ${formatStrength(strengths[1])}`
      : (strengths.length ? `STRENGTH ${formatStrength(strengths[0])}` : entry.label);
