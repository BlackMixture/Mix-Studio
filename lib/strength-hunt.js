...
function strengthHuntSheetLayout(columns, rows, ratio) {
  ...
  const maxWidth = matrix ? 4096 : 6144;
  const maxHeight = 4096;
  const desiredTileWidth = matrix ? 360 : 512;
...
function buildStrengthHuntSheet(images, info = {}) {
  ...
  const { matrix, tileWidth, imageHeight, labelHeight, gap, margin, headerHeight, width, height } = layout;
  const pixels = Buffer.alloc(width * height * 4);
