...
const MAX_PROMPT_PACK_BYTES = 64 * 1024 * 1024;
const MAX_PROMPT_PACK_ASSET_BYTES = 48 * 1024 * 1024;
const MAX_PROMPT_PACK_THUMBNAIL_BYTES = 1024 * 1024;
const MAX_PROMPT_PACK_VIDEO_THUMBNAIL_BYTES = 6 * 1024 * 1024;
const MAX_PROMPT_PACK_PRESETS = 200;
const MAX_PROMPT_PACK_CATEGORIES = 12;
const PROMPT_PACK_ACCENTS = new Set(['cyan', 'blue', 'violet', 'rose', 'amber', 'green']);
const PROMPT_PACK_CONTEXTS = new Set(['create', 'edit', 'video', 'video.h3']);
...
function mp4MovieDuration(buffer, movieBox) {
  const boxes = mp4Boxes(buffer, movieBox.payloadStart, movieBox.end);
  const header = boxes?.find((box) => box.type === 'mvhd');
  if (!header || header.payloadStart + 20 > header.end) return null;
  const version = buffer[header.payloadStart];
  const timescaleOffset = header.payloadStart + (version === 1 ? 20 : 12);
  const durationOffset = header.payloadStart + (version === 1 ? 24 : 16);
  if (version !== 0 && version !== 1) return null;
  if (durationOffset + (version === 1 ? 8 : 4) > header.end) return null;
...
function mp4TrackDimensions(buffer, trackBox) {
  const boxes = mp4Boxes(buffer, trackBox.payloadStart, trackBox.end);
  const header = boxes?.find((box) => box.type === 'tkhd');
  if (!header || header.payloadStart + 84 > header.end) return null;
  const version = buffer[header.payloadStart];
  if (version !== 0 && version !== 1) return null;
  const widthOffset = header.payloadStart + (version === 1 ? 88 : 76);
  const heightOffset = widthOffset + 4;
  if (heightOffset + 4 > header.end) return null;
...
function mp4Metadata(buffer) {
  ...
  if (!boxes?.some((box) => box.type === 'ftyp')) return null;
  const movie = boxes.find((box) => box.type === 'moov');
  if (!movie) return null;
  const movieBoxes = mp4Boxes(buffer, movie.payloadStart, movie.end);
  if (!movieBoxes) return null;
  const dimensions = movieBoxes
    .filter((box) => box.type === 'trak')
    .map((box) => mp4TrackDimensions(buffer, box))
    .find(Boolean);
  if (!dimensions) return null;
  const duration = mp4MovieDuration(buffer, movie);
  if (!Number.isFinite(duration) || duration <= 0) return null;
  return { ...dimensions, duration, kind: 'mp4' };
}
...
function thumbnailDimensions(buffer, mime) {
  ...
  if (mime === 'video/mp4') return mp4Metadata(buffer);
  return null;
}
...
function decodeThumbnail(thumbnail, label) {
  ...
  if (mime === 'video/mp4' && (dimensions.duration < 0.5 || dimensions.duration > 20)) {
    throw packError(`${label} animated thumbnail must be between 0.5 and 20 seconds`);
  }
  return {
    buffer,
    mime,
...
function promptPackContexts(value, label = 'Pack contexts') {
  if (value == null) return [];
  if (!Array.isArray(value) || value.length > PROMPT_PACK_CONTEXTS.size) {
    throw packError(`${label} must be a short list`);
  }
  const contexts = [...new Set(value.map((entry) => String(entry || '').trim().toLowerCase()).filter(Boolean))];
  if (contexts.some((entry) => !PROMPT_PACK_CONTEXTS.has(entry))) {
    throw packError(`${label} contains an unsupported generation context`);
  }
  return contexts;
}

function promptPackSourceUrl(value) {
  const sourceUrl = text(value, 'Pack source URL', 500, false);
  if (!sourceUrl) return '';
  let parsed;
  try { parsed = new URL(sourceUrl); } catch { throw packError('Pack source URL must be a valid HTTPS URL'); }
  if (parsed.protocol !== 'https:') throw packError('Pack source URL must use HTTPS');
...
function inspectPromptPackBuffer(buffer) {
  ...
  if (buffer.length > MAX_PROMPT_PACK_BYTES) {
    throw packError(
      `Prompt pack exceeds the ${MAX_PROMPT_PACK_BYTES / (1024 * 1024)} MB limit`,
  ...
  const credits = text(source.credits, 'Pack credits', 300, false);
  const sourceUrl = promptPackSourceUrl(source.sourceUrl);
  const contexts = promptPackContexts(source.contexts);
  ...
  const normalizedCategories = categories.map((category, categoryIndex) => {
    ...
    const normalizedPresets = presets.map((preset, presetIndex) => {
      ...
      if (assetBytes > MAX_PROMPT_PACK_ASSET_BYTES) {
        throw packError(
          `Prompt pack thumbnails exceed the ${MAX_PROMPT_PACK_ASSET_BYTES / (1024 * 1024)} MB decoded limit`,
        );
      }
...
function normalizeInstalledManifest(source, expectedId = '') {
  ...
  const categories = sourceCategories.map((category) => {
    ...
    const presets = (Array.isArray(category.presets) ? category.presets : []).map((preset) => {
      ...
      if (!new RegExp(`^[a-z][a-z0-9-]*-${presetId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\.(?:jpg|png|webp|mp4)$`).test(thumbnailFile)) {
  ...
  return {
    ...
    credits: text(source.credits, 'Pack credits', 300, false),
    sourceUrl: promptPackSourceUrl(source.sourceUrl),
    contexts: promptPackContexts(source.contexts),
...
async function installPromptPack(root, inspected, options = {}) {
  ...
  const versionComparison = current
    ? comparePackVersions(inspected.manifest.version, current.version)
    : 1;
  if (current && versionComparison < 0 && options.allowDowngrade !== true) {
  ...
  if (current && versionComparison <= 0 && options.replace !== true) {
  ...
  try {
    ...
    if (current) {
      backup = path.join(
        root,
        `.update-backup-${current.id}-${crypto.randomUUID()}`,
      );
    ...
    if (backup) {
      await fsp.rm(backup, {
        ...
        // The installed target is already complete. A hidden backup is safer
        // than reporting a failed update after the new version went live.
      });
    }
...
async function removePromptPack(root, id) {
  const pack = await readInstalledPromptPack(root, id);
  if (!pack) throw packError('Prompt pack is not installed', 'prompt_pack_not_found');
  try {
    await fsp.rm(installedPromptPackPath(root, id), {
      recursive: true,
      force: false,
      maxRetries: 3,
      retryDelay: 100,
    ...
    throw packError(`Could not permanently delete "${pack.name}"`, 'prompt_pack_delete_failed');
  }
  return { pack, deleted: true };
}
