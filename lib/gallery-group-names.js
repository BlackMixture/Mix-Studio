...
function updateGalleryGroupName(items, options = {}) {
  ...
  if (normalizeGenerationName(options.expectedName) !== group.name) {
    return { ok: false, reason: 'stale' };
  }
  const expectedIds = [...new Set((Array.isArray(options.memberIds) ? options.memberIds : []).map(String))].sort();
  const currentIds = group.members.map((item) => String(item.id)).sort();
