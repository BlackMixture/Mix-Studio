...
function h3TurboPreset(settings = {}) {
  ...
  return Object.values(H3_TURBO_PRESETS).find((preset) => (
    ...
    framesSteps: turboBasename(configuredH3TurboLora(settings, 'frames')) === turboBasename(H3_TURBO_LORAS.legacyFrames) ? 4 : 6,
    referenceSteps: 6,
  });
}
...
function h3TurboUsesStandardLoader(settings = {}, mode = 'frames') {
  ...
  return turboBasename(configuredH3TurboLora(settings, mode)) === turboBasename(H3_TURBO_LORAS.lightx8);
}

function h3TurboReferenceIsExperimental(settings = {}) {
  return turboBasename(configuredH3TurboLora(settings, 'reference')) === turboBasename(H3_TURBO_LORAS.lightx8);
}
