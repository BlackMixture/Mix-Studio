...
const { localizedEditPrompt, maskExpand } = require('./edit-mask');
...
function normalizeRegions(input) {
  ...
  source.forEach((region, index) => {
    ...
    const refinedDescription = cleanText(region.refinedDescription || region.refinedPrompt);
...
function elementDesc(region) {
  const desc = region.refinedDescription || region.description || 'subject';
...
function addBaseLoaders(graph, settings) {
  const s = settings || {};
  graph.unet = buildKrea2ModelLoader(s, s.unet);
...
function buildRegionalT2IGraph(params) {
  ...
  graph.neg = p.negativePrompt
    ? { class_type: 'CLIPTextEncode', inputs: { clip: regional.clip, text: p.negativePrompt } }
...
function buildKrea2InpaintGraph(params) {
  ...
  let clip = chain.clip;
  let textSource = localizedEditPrompt(cleanText(p.enhancedText || p.prompt));
  ...
  // A modest creative collar lets the model rebuild transitions such as
  // cast shadows and material texture instead of pasting a rigid patch into
  // the exact selection outline. The same soft, grown mask is used to blend
  // that collar back into the untouched source.
  graph.grow_mask = { class_type: 'GrowMask', inputs: { mask: ['mask', 0], expand: maskExpand(p.maskExpand), tapered_corners: true } };
  ...
  if (p.useSourceConditioning && !regions.length) {
    graph.pos = {
      ...
      inputs: {
        ...
        image1: ['source', 0],
        image1_tokens: 'high',
        // Conditioning-Rebalance renamed these controls. The server filters
        // this graph against the installed node schema before it is queued.
  ...
  graph.sampler = {
    ...
    inputs: {
      ...
      sampler_name: 'euler',
      scheduler: 'beta',
      denoise: clampNumber(p.denoise, 0.05, 1, 0.78),
    },
  };
  graph.decode = { class_type: 'VAEDecode', inputs: { samples: ['sampler', 0], vae: ['vae', 0] } };
  graph.composite = {
  ...
  graph.save = { class_type: 'SaveImage', inputs: { images: ['composite', 0], filename_prefix: 'KreaStudio/edit' } };
  return graph;
}
