...
function publicProbe(snapshot, plan, status = {}) {
  ...
  return {
    ...
    // Reached only with an interpreter in hand, so everything below was measured on
    // this machine. Callers that must not confuse "measured and unusable" with "never
    // measured" read this field rather than ready/pythonReady.
    verifiable: true,
...
async function probeSageAttention(runtime, options = {}) {
  ...
  if (!pythonPath || !(options.existsSync || fs.existsSync)(pythonPath)) {
    return {
      ...
      installable: false,
      pythonReady: false,
      // ready/pythonReady read false here for the same reason they read false for a
      // verified-broken install, but nothing was inspected: the probe runs on this
      // machine's filesystem, so a ComfyUI on another host can never reach it.
      verifiable: false,
      reason: 'Mix Studio has no local ComfyUI Python environment to inspect, so SageAttention cannot be verified on this machine. This is expected when ComfyUI runs on another host.',
    };
  }
  const now = Date.now();
  ...
  try {
    ...
    cachedProbe = {
      ...
      installable: false,
      pythonReady: true,
      // The probe had an interpreter and ran; it just could not produce a report.
      // That is a failure to install or run SageAttention, not a failure to look.
      verifiable: true,
...
// Whether the MiniMax H3 route may refuse a render that asked for SageAttention, and
// which attention backend that render will then use. This lives here, beside the probe
// it interprets, because the distinction it encodes is the probe's own: an optional
// accelerator must never block a render on the strength of a check that never ran.
//
//   ready      — verifiable by construction. The probe imports the package and runs a
//                real CUDA kernel, so this is the only field that turns SageAttention
//                on; node presence or a local install is never enough on its own.
//   verifiable — false only when no interpreter could be inspected, which is the
//                normal outcome when ComfyUI runs on another host.
//   nodeReady  — read from the CONNECTED ComfyUI's object_info, so it holds even when
//                the local probe cannot run, and the graph cannot be built without the
//                patch node.
//
// Unverifiable falls back to standard attention rather than refusing: ready stays false,
// so the render is honest about which backend it used, and a probe that ran and found
// SageAttention unusable still refuses.
function h3SageAttentionDecision(sageRuntime, nodeReady) {
  const runtime = sageRuntime || {};
  if (nodeReady !== true) return { block: true, sageAttention: false };
  // Checked before ready: a report that cannot describe this machine must never turn
  // the accelerator on, so ready === true only counts alongside verifiable === true.
  // (The probe cannot currently produce that pair, and this keeps it that way.)
  if (runtime.verifiable === false) return { block: false, sageAttention: false };
  if (runtime.ready === true) return { block: false, sageAttention: true };
  return { block: true, sageAttention: false };
}
...
async function installSageAttention(runtime, report = () => {}, options = {}) {
  ...
  if (initialProbe.ready) {
    ...
    return Object.assign({}, cachedProbe, { changed: false });
  }
