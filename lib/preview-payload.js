...
function decodePreviewPayload(buffer) {
  ...
  let metadata = null;
  if (eventType === 1) {
    format = buffer.readUInt32BE(4);
    image = buffer.subarray(8);
  } else if (eventType === 4) {
    const metadataLength = buffer.readUInt32BE(4);
    const imageOffset = 8 + metadataLength;
    if (imageOffset >= buffer.length) return null;
    try { metadata = JSON.parse(buffer.subarray(8, imageOffset).toString('utf8')); } catch { return null; }
    image = buffer.subarray(imageOffset);
  } else {
    return null;
  }
  if (!image.length) return null;
  return { mime: imageMime(image, format), image, metadata };
}
