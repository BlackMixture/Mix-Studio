...
function ltx25Compatibility(info, version = '') {
  ...
  // LTX 2.5 landed after ComfyUI 0.31.0 and its official core PR was still
  // open on release day. Never infer support from a version number: a build
  // is ready only when it exposes the new native graph contract itself.
...
function ltx25CompatibilityError(compatibility = {}) {
  const version = normalizeVersion(compatibility.version);
  return `This${version ? ` ComfyUI ${version}` : ' ComfyUI build'} does not expose native LTX 2.5 support yet. The official ComfyUI support PR is still pending; use an LTX 2.5-capable build or wait for it to merge, then restart ComfyUI and run Check again.`;
}
