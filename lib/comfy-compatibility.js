...
const COMFYUI_RELEASES_URL = 'https://github.com/Comfy-Org/ComfyUI/releases';
...
function ltx25Compatibility(info, version = '') {
  ...
  // LTX 2.5 landed after ComfyUI 0.31.0. Keep capability detection
  // authoritative because custom and partially updated builds can report a
  // new version without exposing the complete native graph contract.
  const supported = info && typeof info === 'object'
    ? missingNodes.length === 0 && !missingClipType
    : null;
  return {
    version: normalizedVersion,
    updateUrl: COMFYUI_RELEASES_URL,
    supported,
    missingNodes,
    missingClipType,
  };
