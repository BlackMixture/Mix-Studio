...
function childDirectoriesMatching(root, pattern, fsImpl = fs, pathApi = path) {
  if (!root) return [];
  try {
    return fsImpl.readdirSync(root, { withFileTypes: true })
      .filter((entry) => entry && entry.isDirectory() && pattern.test(String(entry.name || '')))
...
/*
 * ComfyUI Portable is commonly unpacked as
 * ComfyUI_windows_portable/ComfyUI rather than ~/ComfyUI. Keep discovery
 * shallow and predictable: inspect the user's usual download locations, the
 * Mix Studio checkout's parent, and conventional folders on the known drive.
 */
function portableInstallationCandidates(runtime = {}, options = {}) {
  const env = options.env || process.env;
  const fsImpl = options.fsImpl || fs;
  const pathApi = options.pathApi || path;
  const home = String(options.home || env.HOME || env.USERPROFILE || '').trim();
  const appRoot = String(options.appRoot || runtime?.appRoot || '').trim();
  const userRoots = uniquePaths([
    home,
    home ? pathApi.join(home, 'Desktop') : '',
    home ? pathApi.join(home, 'Downloads') : '',
    home ? pathApi.join(home, 'Documents') : '',
    appRoot ? pathApi.dirname(appRoot) : '',
    ...(Array.isArray(options.searchRoots) ? options.searchRoots : []),
  ], pathApi);
  const knownDriveRoots = uniquePaths([
    String(env.HOMEDRIVE || '').trim(),
    String(env.SystemDrive || '').trim(),
    appRoot && typeof pathApi.parse === 'function' ? pathApi.parse(appRoot).root : '',
  ], pathApi);
  const named = userRoots.flatMap((root) => [
    pathApi.join(root, 'ComfyUI'),
    pathApi.join(root, 'ComfyUI_windows_portable'),
    ...childDirectoriesMatching(root, /^ComfyUI(?:$|[\s_-])/i, fsImpl, pathApi),
  ]).concat(knownDriveRoots.flatMap((root) => [
    pathApi.join(root, 'ComfyUI'),
    pathApi.join(root, 'ComfyUI_windows_portable'),
  ]));
  return uniquePaths(named.flatMap((candidate) => [
    candidate,
    pathApi.join(candidate, 'ComfyUI'),
  ]), pathApi);
}

function appDataRoots(env = process.env, pathApi = path, platform = process.platform, home = '') {
  const userHome = String(home || env.HOME || env.USERPROFILE || '').trim();
  const explicitAppData = String(env.APPDATA || '').trim();
  if (platform === 'win32') return [explicitAppData].filter(Boolean);
  const roots = explicitAppData ? [explicitAppData] : [];
  if (platform === 'darwin') {
    if (userHome) roots.push(pathApi.join(userHome, 'Library', 'Application Support'));
    return [...new Set(roots)];
  }
  const xdgConfig = String(env.XDG_CONFIG_HOME || '').trim();
  roots.push(xdgConfig || (userHome ? pathApi.join(userHome, '.config') : ''));
  return [...new Set(roots.filter(Boolean))];
}

function desktopBasePath(env, fsImpl = fs, pathApi = path, platform = process.platform, home = '') {
...
function comfyDesktop2Installations(env, fsImpl = fs, pathApi = path, platform = process.platform, home = '') {
  for (const root of appDataRoots(env, pathApi, platform, home)) {
...
function comfyDesktopInstallations(env, fsImpl = fs, pathApi = path, platform = process.platform, home = '') {
  let records = [];
  for (const root of appDataRoots(env, pathApi, platform, home)) {
  ...
  return records
    ...
    .concat(comfyDesktop2Installations(env, fsImpl, pathApi, platform, home));
}
...
function desktopInstallationCandidates(env, fsImpl = fs, pathApi = path, platform = process.platform, home = '') {
  return comfyDesktopInstallations(env, fsImpl, pathApi, platform, home).flatMap((record) => {
...
function findComfyBase(runtime = {}, options = {}) {
  ...
  const registered = desktopInstallationCandidates(env, fsImpl, pathApi, platform, home);
  const candidates = uniquePaths([
    env.COMFYUI_PATH,
    comfy.path,
    ...registered.map((candidate) => candidate.basePath),
    desktopBasePath(env, fsImpl, pathApi, platform, home),
    comfy.modelsPath ? pathApi.dirname(comfy.modelsPath) : '',
    ...portableInstallationCandidates(runtime, Object.assign({}, options, {
      env, fsImpl, home, pathApi, platform,
    })),
...
function findComfyDataBase(runtime = {}, options = {}) {
  ...
  for (const record of comfyDesktopInstallations(env, fsImpl, pathApi, platform, home)) {
...
function findComfyPython(basePath, options = {}) {
  ...
  const registeredPython = desktopInstallationCandidates(env, fsImpl, pathApi, platform, home)
...
module.exports = {
  ...
  portableInstallationCandidates,
  findComfyBase,
  findComfyDataBase,
