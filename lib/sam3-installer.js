...
function sam3InstallStatus(runtime, options = {}) {
  ...
  return {
    ...
    reason: !basePath
      ...
      : (!pythonPath ? 'MixBox Studio could not find the Python environment used by this ComfyUI installation.' : ''),
  };
}
...
async function installSam3(runtime, options = {}) {
  ...
  if (existsSync(requirements)) {
    ...
    if (uv) {
      await run(uv, ['pip', 'install', '--python', status.pythonPath, '--upgrade', '-r', requirements], { cwd: status.nodePath });
    } else {
      await run(status.pythonPath, ['-m', 'pip', 'install', '--upgrade', '-r', requirements], { cwd: status.nodePath });
    }
  }
