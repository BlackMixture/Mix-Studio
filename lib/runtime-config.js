...
function resolveRuntimeConfig(root, options = {}) {
  ...
  return {
    ...
    update: {
      ...
      releaseChannel: ['stable', 'preview', 'development'].includes(env.MIXBOX_RELEASE_CHANNEL || update.releaseChannel)
        ? (env.MIXBOX_RELEASE_CHANNEL || update.releaseChannel) : 'stable',
      gitExecutable,
    },
    comfy: {
...
function publicAnalyticsConfig(runtime) {
...
function publicRuntimeConfig(runtime) {
  return {
    ...
    update: {
      ...
      releaseChannel: runtime.update?.releaseChannel || 'stable',
    },
