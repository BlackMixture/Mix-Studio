...
const COMPATIBLE_MODEL_NAMES = Object.freeze({
  // Existing Krea 2 installations use both of these Qwen3-VL 4B FP8
  // filenames. Keep either registered choice instead of downloading a second
  // 4B encoder solely because its filename differs.
  clip: Object.freeze([
...
function resolveRegisteredModelSetting(settingKey, value, availableNames) {
  const configured = resolveRegisteredModelName(value, availableNames);
  if (configured) return configured;
  for (const compatible of COMPATIBLE_MODEL_NAMES[settingKey] || []) {
    const resolved = resolveRegisteredModelName(compatible, availableNames);
