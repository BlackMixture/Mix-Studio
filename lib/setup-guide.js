...
function featureHardwareFit(feature = {}, hardwareInfo = {}) {
  ...
  const minimumRam = Number(rules.minimumRamGb || 0);
  const recommendedRam = Number(rules.recommendedRamGb || minimumRam);
  ...
  if (!profile.vramGb) {
    return { level: 'unknown', label: 'VRAM unknown', detail: `${variant}. Requires at least ${minimumVram} GB VRAM; ${recommendedVram} GB is recommended.`, variant };
  }
  const belowMinimum = profile.vramGb < minimumVram || (profile.memoryGb > 0 && profile.memoryGb < minimumRam);
  if (belowMinimum) {
    const issues = [];
    if (profile.vramGb < minimumVram) issues.push(`${minimumVram} GB VRAM minimum; ${profile.vramGb} GB detected`);
    if (profile.memoryGb > 0 && profile.memoryGb < minimumRam) issues.push(`${minimumRam} GB system RAM minimum; ${profile.memoryGb} GB detected`);
    return { level: 'difficult', label: 'Difficult on this PC', detail: `${variant}. ${issues.join('. ')}. Very long runs or out-of-memory errors are likely.`, variant };
  }
  const belowRecommended = profile.vramGb < recommendedVram || (profile.memoryGb > 0 && profile.memoryGb < recommendedRam);
  if (belowRecommended) {
    return { level: 'limited', label: 'Can run with offload', detail: `${variant}. ${recommendedVram} GB VRAM and ${recommendedRam} GB RAM are recommended. Expect slower generation and system-memory use.`, variant };
  }
  return { level: 'recommended', label: 'Recommended for this PC', detail: `${variant}. Meets the ${recommendedVram} GB VRAM and ${recommendedRam} GB RAM recommendation.`, variant };
}
