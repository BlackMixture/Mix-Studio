...
function featureHardwareFit(feature = {}, hardwareInfo = {}) {
  ...
  if (belowMinimum) {
    const issues = [];
    if (profile.vramGb < minimumVram) issues.push(`${minimumVram} GB guided VRAM tier; ${profile.vramGb} GB detected`);
    if (profile.memoryGb > 0 && profile.memoryGb < minimumRam) issues.push(`${minimumRam} GB guided system RAM tier; ${profile.memoryGb} GB detected`);
    return { level: 'difficult', label: 'Below guided tier', detail: `${variant}. ${issues.join('. ')}. Installation remains available, but stronger offloading, very long runs, or out-of-memory errors are likely.`, variant };
  }
