...
async function buildWanAnimate2Graph(referenceImageName, opts = {}, settings = {}, deps = {}) {
  ...
  graph.performance_slice = {
    class_type: 'Video Slice',
    inputs: {
      video: ['performance_load', 0],
      start_time: Math.max(0, Number(segment.startSeconds) || 0),
      duration: WAN_ANIMATE_2_FRAMES / Math.max(1, Number(plan.fps) || 24),
      strict_duration: false,
    },
  };
  graph.performance_parts = {
    class_type: 'GetVideoComponents',
    inputs: { video: ['performance_slice', 0] },
  };
