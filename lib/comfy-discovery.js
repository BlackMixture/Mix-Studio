...
function desktopRecordsForBase(basePath, options = {}) {
  ...
  const pathApi = options.pathApi || path;
  return comfyDesktopInstallations(
    env, fsImpl, pathApi, options.platform || process.platform, options.home || '',
  ).filter((record) => (
...
function desktopPortLocks(options = {}) {
  ...
  let names = [];
  let directory = '';
  for (const root of appDataRoots(env, pathApi, options.platform || process.platform, options.home || '')) {
