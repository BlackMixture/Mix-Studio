...
const PROVIDERS = Object.freeze(['openai', 'gemini', 'ollama']);
...
function normalizeExternalLlmSettings(source = {}) {
  ...
  return {
    ...
    externalLlmImageRevise: source.externalLlmImageRevise === true,
    externalLlmImageEnhance: source.externalLlmImageEnhance === true,
    externalLlmVideoRevise: source.externalLlmVideoRevise === true,
    externalLlmVideoEnhance: source.externalLlmVideoEnhance === true,
  };
}

function externalLlmEnabled(settings, domain, action) {
  const domainName = domain === 'video' ? 'Video' : 'Image';
  const actionName = action === 'enhance' ? 'Enhance' : 'Revise';
  return settings?.[`externalLlm${domainName}${actionName}`] === true;
}
...
function providerErrorDetail(payload, fallback = '') {
  ...
  return String(detail || '').trim().slice(0, 600);
}
...
async function externalLlmRequest(options = {}) {
  ...
  try {
    response = await fetchImpl(url, {
      ...
      signal: options.signal || (typeof AbortSignal !== 'undefined' && typeof AbortSignal.timeout === 'function'
