...
const PROVIDERS = Object.freeze(['openai', 'gemini', 'ollama', 'lmstudio', 'local']);
...
const DEFAULTS = Object.freeze({
  ...
  externalLlmLmStudioModel: '',
  externalLlmLmStudioUrl: 'http://127.0.0.1:1234/v1',
...
function normalizeLmStudioUrl(value) {
  const raw = cleanText(value, DEFAULTS.externalLlmLmStudioUrl, 1000);
  let url;
  try { url = new URL(raw); } catch {
    throw new Error('LM Studio URL must be a valid HTTP or HTTPS address');
  }
  if (!['http:', 'https:'].includes(url.protocol)) {
    throw new Error('LM Studio URL must use HTTP or HTTPS');
  }
  if (url.username || url.password) throw new Error('LM Studio URL cannot contain credentials');
  url.search = '';
  url.hash = '';
  url.pathname = url.pathname.replace(/\/+$/, '') || '/';
  return url.toString().replace(/\/$/, '');
...
function lmStudioChatUrl(value) {
  const base = normalizeLmStudioUrl(value);
...
function normalizeExternalLlmSettings(source = {}) {
  ...
  const localProvider = ['local', 'ollama', 'lmstudio'].includes(provider)
    ? provider
    : (['local', 'ollama', 'lmstudio'].includes(source.externalLlmLocalProvider)
  ...
  let lmStudioUrl;
  try { lmStudioUrl = normalizeLmStudioUrl(source.externalLlmLmStudioUrl); }
  catch { lmStudioUrl = DEFAULTS.externalLlmLmStudioUrl; }
  return {
    ...
    externalLlmLmStudioModel: cleanText(source.externalLlmLmStudioModel, DEFAULTS.externalLlmLmStudioModel),
    externalLlmLmStudioUrl: lmStudioUrl,
    externalLlmImageRevise: true,
    externalLlmImageEnhance: true,
    externalLlmVideoRevise: true,
    externalLlmVideoEnhance: true,
...
function externalLlmProviderConfig(settings = {}) {
  ...
  if (provider === 'lmstudio') return {
    provider,
    label: PROVIDER_LABELS[provider],
    model: normalized.externalLlmLmStudioModel,
    baseUrl: normalized.externalLlmLmStudioUrl,
    apiKey: '',
  };
  return {
    provider,
...
function outputText(payload) {
  ...
  const compatible = Array.isArray(payload?.choices)
    ? payload.choices.map((choice) => choice?.message?.content)
...
async function externalLlmRequest(options = {}) {
  ...
  // Smart production plans can legitimately exceed 4K tokens. Keep a generous
  // safety ceiling while preserving the smaller budgets used by prompt polish.
  const maxTokens = Math.max(64, Math.min(16384, Math.round(Number(options.maxTokens) || 512)));
  ...
  if (!['ollama', 'lmstudio'].includes(provider) && !apiKey) throw new Error(`Add ${provider === 'openai' ? 'an' : 'a'} ${label} API key in Preferences`);
  ...
  if (provider === 'openai') {
    ...
    url = lmStudioChatUrl(options.baseUrl);
  ...
  const timeoutCeilingMs = ['ollama', 'lmstudio'].includes(provider) ? 90 * 60_000 : 15 * 60_000;
  const timeoutMs = Math.max(1000, Math.min(timeoutCeilingMs,
    Math.round(Number(options.timeoutMs) || 180_000)));
