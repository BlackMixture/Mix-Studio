...
async function buildMiniMaxH3Graph(opts = {}, settings = {}, deps = {}) {
  ...
  const turbo = opts.turbo === true && mode === 'frames';
  const steps = turbo ? 4 : Math.max(1, Math.min(100, Math.round(Number(opts.steps) || 20)));
  const turboStrength = Math.max(-10, Math.min(10, Number.isFinite(Number(opts.turboStrength))
  ...
  const graph = {
    ...
    scheduler: {
      ...
      inputs: { model: [turbo ? 'turbo_lora' : 'model', 0], scheduler: 'simple', steps, denoise: 1 },
    },
  };
  ...
  if (opts.sageAttention) {
    graph.sage_attention = {
      ...
      inputs: {
        model: [turbo ? 'turbo_lora' : 'model', 0],
        sage_attention: 'auto',
        allow_compile: false,
      },
    };
  ...
  graph.guider = {
    ...
    inputs: {
      model: opts.sageAttention ? ['sage_attention', 0] : [turbo ? 'turbo_lora' : 'model', 0],
  ...
  graph.sample = {
    ...
    inputs: {
      noise: ['noise', 0],
      guider: ['guider', 0],
      sampler: [turbo ? 'turbo_sampler' : 'sampler_select', 0],
      sigmas: ['scheduler', 0],
