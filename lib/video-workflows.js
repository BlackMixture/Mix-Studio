...
const { h3EffectiveModelName, h3LoraCompatibility, h3TurboCompatibility } = require('./h3-model-variants');
const {
  h3TurboDefaultSteps,
  h3TurboSigmaShifts,
  h3TurboUsesStandardLoader,
} = require('./h3-turbo-presets');
const { h3AttentionOptions } = require('./h3-attention');
...
function h3ReferenceList(value, limit) {
  return (Array.isArray(value) ? value : [])
    ...
    .map((asset) => ({
      ...
      dur: Math.max(0, Math.min(86400, Number(asset.dur) || 0)),
      trimStart: Math.max(0, Math.min(86400, Number(asset.trimStart) || 0)),
      trimEnd: Math.max(0, Math.min(86400, Number(asset.trimEnd) || 0)),
    }));
}
...
async function buildMiniMaxH3Graph(opts = {}, settings = {}, deps = {}) {
  ...
  const attention = h3AttentionOptions(
    opts.attentionBackend,
    Object.prototype.hasOwnProperty.call(opts, 'sageAttention') ? opts.sageAttention : false,
  );
  ...
  const standardTurboLoader = turbo && h3TurboUsesStandardLoader(settings, mode);
  const turboShifts = turbo ? h3TurboSigmaShifts(settings, mode) : null;
  const userLoras = (Array.isArray(opts.loras) ? opts.loras : [])
    .filter((lora) => lora && lora.on && lora.name && String(lora.name).trim());
  const loraCompatibility = h3LoraCompatibility(settings, mode);
  if (userLoras.length && loraCompatibility.supported !== true) {
    const error = new Error(loraCompatibility.reason);
    error.code = 'h3_lora_model_incompatible';
    throw error;
  }
  const steps = turbo
    ? Math.max(4, Math.min(100, Math.round(Number(opts.steps) || h3TurboDefaultSteps(settings, mode))))
    : Math.max(1, Math.min(100, Math.round(Number(opts.steps) || 20)));
  const turboNativeSampler = turbo && opts.turboNativeSampler === true;
  ...
  // The managed Turbo adapter establishes the workflow's sampling behavior.
  // User-selected H3 LoRAs are applied after it so ComfyUI's model clone keeps
  // the custom Turbo injection as well as every ordinary model patch.
  if (turbo) {
    if (standardTurboLoader) {
      // LightX2V's ComfyUI checkpoints use the generic diffusion_model.* LoRA
      // layout. Keep them on ComfyUI's stock model-only loader. This covers the
      // curated Reference adapter and the optional v1.0 eight-step adapter.
      graph.turbo_lora = {
        class_type: 'LoraLoaderModelOnly',
        inputs: {
          model: ['model', 0],
          lora_name: referenceTurbo ? settings.h3RefTurboLora : settings.h3TurboLora,
  ...
  // Ordinary H3 LoRAs are model-only adapters. Keep their order stable and
  // preserve zero/negative strengths used for testing or style subtraction.
  let userModelNode = turbo ? 'turbo_lora' : 'model';
  let userLoraIndex = 0;
  for (const lora of userLoras) {
    const name = String(lora.name).trim();
    if (!name) continue;
    const requestedStrength = Number(lora.strength);
    const strength = Number.isFinite(requestedStrength)
      ? Math.max(-100, Math.min(100, requestedStrength))
      : 0;
    userLoraIndex += 1;
    const key = `user_lora_${userLoraIndex}`;
    graph[key] = {
      class_type: 'LoraLoaderModelOnly',
      inputs: {
  ...
  let samplingModelNode = userModelNode;
  if (turbo) {
    if (standardTurboLoader || turboNativeSampler) {
      graph.turbo_sampling = {
        class_type: 'MiniMaxH3SigmaShift',
        inputs: {
          model: [userModelNode, 0],
          shift_video: turboShifts.video,
          shift_audio: turboShifts.audio,
        },
      };
      samplingModelNode = 'turbo_sampling';
    }
    if (turboNativeSampler) {
      // Current ComfyUI maps H3 audio onto the video schedule natively. Stock
      // Euler is therefore audio-safe for both Frames and Reference Turbo.
  ...
  if (attention.attentionBackend === 'kitchen') {
    graph.kitchen_attention = { class_type: 'ModelAttentionBackend', inputs: { model: [samplingModelNode, 0], attention: 'comfy kitchen attention' } };
  } else if (attention.sageAttention) {
  ...
  } else if (attention.slaAttention) {
    graph.sla_attention = {
  ...
  const patchedModelNode = attention.attentionBackend === 'kitchen' ? 'kitchen_attention' : attention.sageAttention
    ? 'sage_attention'
    : (attention.slaAttention ? 'sla_attention' : samplingModelNode);
  graph.scheduler = {
    class_type: 'BasicScheduler',
    inputs: {
      model: attention.slaAttention ? ['sla_attention', 0] : [samplingModelNode, 0],
      scheduler: 'simple', steps, denoise: 1,
    },
  };
  ...
  const turboReferenceSegment = suppliedTurboReferenceSegment
    ...
    || (turboReferencePlan.length === 1 ? turboReferencePlan[0] : null)
    || null;
  ...
  const addReferenceCondition = async ({ conditionKey, segment = null }) => {
    ...
    for (let index = 0; index < references.videos.length; index += 1) {
      ...
      const trimStartFrame = Math.max(0, Math.round((Number(asset.trimStart) || 0) * H3_FPS));
      const segmentStartFrame = segment ? segment.startFrame : 0;
      const skipFirstFrames = trimStartFrame + segmentStartFrame;
      const requestedFrames = segment ? segment.generationFrames : opts.frames;
      const trimEndFrame = Number(asset.trimEnd) > Number(asset.trimStart)
        ? Math.max(trimStartFrame + 1, Math.round(Number(asset.trimEnd) * H3_FPS))
        : 0;
      const frameLoadCap = trimEndFrame
        ? Math.max(1, Math.min(requestedFrames, trimEndFrame - skipFirstFrames))
        : requestedFrames;
  ...
  const addSamplingBranch = (conditionKey, latentConditionKey = conditionKey, index = null) => {
    ...
    graph[sampleKey] = {
      ...
      inputs: {
        noise: ['noise', 0],
        guider: [guiderKey, 0],
        sampler: [turbo && !turboNativeSampler ? 'turbo_sampler' : 'sampler_select', 0],
        sigmas: ['scheduler', 0],
