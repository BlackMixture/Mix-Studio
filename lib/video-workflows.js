...
async function buildMiniMaxH3Graph(opts = {}, settings = {}, deps = {}) {
  ...
  const turboReferenceSegment = turboReferencePlan.length
    ? turboReferencePlan.find((segment) => segment.index === requestedTurboSegmentIndex)
    : null;
  if (turboReferencePlan.length && !turboReferenceSegment) {
  ...
  const addSamplingBranch = (conditionKey, index = null) => {
    ...
    graph[videoKey] = {
      ...
      inputs: { images, audio: [decodeAudioKey, 0], fps: H3_FPS, crf: 8 },
    };
    graph[saveKey] = {
  ...
  const posterSource = addSamplingBranch('condition');
  if (opts.makePoster) {
    graph.poster_pick = {
