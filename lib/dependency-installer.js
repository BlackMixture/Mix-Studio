'use strict';
const { QWEN21_REPO, qwen21VariantSettings } = require('./qwen21-workflows');
...
const MODEL_ASSETS = Object.freeze({
  promptAi: [
    ['localPromptAiClip', 'text_encoders', hf('cusiman/Huihui-Qwen3-VL-4B-Instruct-abliterated-comfy', 'Huihui-Qwen3-VL-4B-Instruct-abliterated-fp8_scaled.safetensors')],
  ],
  image: [
  ...
  qwen21: [
    ['qwen21Unet', 'diffusion_models', `${QWEN21_REPO}/resolve/main/diffusion_models/qwen_image_2.1_int8_convrot.safetensors`],
    ['qwen21Clip', 'text_encoders', `${QWEN21_REPO}/resolve/main/text_encoders/qwen3vl_8b_w4a8.safetensors`],
    ['qwen21Vae', 'vae', `${QWEN21_REPO}/resolve/main/vae/qwen_image_2.1_vae_bf16.safetensors`],
  ],
  qwen: [
...
function dependencyModelPlan(modelGroups, settings = {}, options = {}) {
  ...
  if (groups.includes('qwen21')) Object.assign(settingUpdates, qwen21VariantSettings(options.modelVariants?.qwen21 || settings.qwen21ModelVariant, options));
  ...
  const promptAiConfigured = String(effectiveSettings.localPromptAiClip || effectiveSettings.clip || '').trim();
  const promptAiBasename = normalizeModelName(promptAiConfigured).split('/').pop();
  const reviewedPromptAiNames = new Set([
    'huihui-qwen3-vl-4b-instruct-abliterated-fp8_scaled.safetensors',
    'qwen3vl_4b_fp8_scaled.safetensors',
  ]);
  let promptAiAsset = effectiveSettings.localPromptAiClip
    ? MODEL_ASSETS.promptAi[0]
    : ['clip', ...MODEL_ASSETS.promptAi[0].slice(1)];
  if (promptAiBasename && !reviewedPromptAiNames.has(promptAiBasename)) {
    promptAiAsset = [
      promptAiAsset[0], promptAiAsset[1], promptAiAsset[2], undefined, undefined,
      { checkOnly: true, customKind: 'Prompt AI text encoder' },
    ];
  }
  const assets = [...new Map(groups.flatMap((key) => (MODEL_ASSETS[key] || []).map((asset) => {
    ...
    if (key === 'qwen21') return [asset[0], asset[1], `${QWEN21_REPO}/resolve/main/${asset[1]}/${effectiveSettings[asset[0]]}`];
    if (key === 'promptAi') return promptAiAsset;
    return asset;
...
const COMPONENTS = Object.freeze({
  promptai: { label: 'Local Prompt AI', nodes: ['kjnodes'], models: ['promptAi'] },
  ...
  qwen21: { label: 'Qwen Image 2.1 · Create + Edit', nodes: [], models: ['qwen21'] },
...
async function downloadAsset(asset, modelsPath, settings, report, options = {}) {
  ...
  if (policy?.checkOnly === true) {
    const customKind = String(policy.customKind || 'MiniMax H3 Turbo adapter');
    const error = new Error(
      `${filename} is a custom ${customKind} and was not found. `
      + `Install that exact file in ComfyUI first, then run Check again; Mix Studio will not download or rename a different ${customKind} into its place.`
    );
