...
const NODE_PACKS = Object.freeze({
  gguf: { label: 'ComfyUI GGUF Loader', folder: 'ComfyUI-GGUF', repo: 'https://github.com/city96/ComfyUI-GGUF.git' },
  ...
  ltxvideo: { label: 'LTXVideo', folder: 'ComfyUI-LTXVideo', repo: 'https://github.com/Lightricks/ComfyUI-LTXVideo.git' },
  ...
  krea2Style: { label: 'Krea 2 Style Transfer', folder: 'ComfyUI-Krea2-StyleTransfer', repo: 'https://github.com/jieg9341-lab/ComfyUI-Krea2-StyleTransfer.git' },
...
function dependencyModelPlan(modelGroups, settings = {}, options = {}) {
  ...
  const settingUpdates = groups.includes('image') && requested ? krea2VariantSettings(krea2Variant) : {};
  ...
  const assets = [...new Map(groups.flatMap((key) => (MODEL_ASSETS[key] || []).map((asset) => {
    if (key === 'image' && variantAssets.has(asset[0])) return variantAssets.get(asset[0]);
...
async function installNodePack(pack, status, report, options = {}) {
  ...
  if (nodeExists && !gitManaged) {
    ...
    if (pack.ref) {
      report('existing-node', `Using the existing compatible ${pack.label} checkout without changing it`, { pinnedRef: pack.ref });
    } else {
    ...
    await runCommand('git', cloneArgs, { cwd: status.customNodesPath, signal: options.signal });
    if (pack.ref) {
      await runCommand('git', ['-C', nodePath, 'checkout', '--detach', pack.ref], { cwd: nodePath, signal: options.signal });
    }
  }
...
async function downloadAsset(asset, modelsPath, settings, report, options = {}) {
  ...
  for (const root of [...new Set(Array.isArray(options.availableModelRoots) ? options.availableModelRoots : [])]) {
    ...
    if (root && fs.existsSync(existing) && fs.statSync(existing).size > 0) {
