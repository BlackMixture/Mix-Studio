...
async function snapshotPythonEnvironment(status, runtime, report, options = {}) {
  ...
  try {
    ...
    const freeze = await runCommand(status.pythonPath, ['-m', 'pip', 'freeze'], { cwd: status.basePath });
...
async function installNodePack(pack, status, report, options = {}) {
  ...
  if (existsSync(gitDir)) {
    ...
    const origin = await runCommand('git', ['-C', nodePath, 'remote', 'get-url', 'origin'], { cwd: nodePath });
    if (!sameRepo(origin, pack.repo)) throw new Error(`${pack.label} exists but points to a different repository.`);
    await runCommand('git', ['-C', nodePath, 'pull', '--ff-only'], { cwd: nodePath });
  } else {
    report('installing-node', `Installing ${pack.label}`);
    await runCommand('git', ['clone', '--depth', '1', pack.repo, nodePath], { cwd: status.customNodesPath });
  }
  ...
  if (existsSync(installScript)) {
    ...
    await runCommand(status.pythonPath, [installScript], { cwd: nodePath });
  }
}

async function downloadAsset(asset, modelsPath, settings, report, options = {}) {
  const [settingKey, folder, url] = asset;
  const filename = cleanRelative(settings[settingKey]);
  ...
  const response = await (options.fetch || fetch)(url, { headers, redirect: 'follow' });
...
async function installComponents({ runtime, settings, components, report = () => {}, options = {} }) {
  ...
  if (!status.canInstall) {
    const error = new Error(status.reason || 'MixBox Studio could not find a writable ComfyUI installation and its Python environment.');
  ...
  for (const id of nodeIds) {
    ...
    await installNodePack(pack, status, (phase, message, detail) => report(phase, message, Object.assign({ completed, total, component: id }, detail || {})), options);
    completed += 1;
  ...
  if (nodeIds.length) {
    try {
      ...
      await (options.run || run)(status.pythonPath, ['-m', 'pip', 'check'], { cwd: status.basePath });
    } catch (error) {
