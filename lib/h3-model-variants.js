...
function h3LoraCompatibility(settings = {}, mode = 'frames') {
  const variant = mode === 'reference' ? h3ReferenceVariant(settings) : h3FrameVariant(settings);
  return variant.requiresPatch !== true
    ? { supported: true, variant: variant.id, reason: '' }
    : {
      supported: false,
      variant: variant.id,
      reason: 'Current MiniMax H3 LoRAs target the standard fused QKV model layout. DynTime uses separate Q/K/V projections, so choose Standard / Full BF16 or remove the LoRA stack.',
    };
}
