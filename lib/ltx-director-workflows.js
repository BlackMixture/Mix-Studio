...
function normalizeExtensionSource(value) {
  ...
  const inputNameValue = String(value.inputName || '').trim();
  const itemId = String(value.itemId || '').trim();
  const videoId = String(value.videoId || '').trim();
  if (inputNameValue) {
    if (itemId || videoId) throw new Error('Director extension source is invalid');
    const inputName = normalizeDirectorAssetName(inputNameValue, 'video');
    const sourceSeconds = value.sourceSeconds == null || value.sourceSeconds === ''
      ? undefined : Number(value.sourceSeconds);
    const sourceWidth = value.sourceWidth == null || value.sourceWidth === ''
      ? undefined : Number(value.sourceWidth);
    const sourceHeight = value.sourceHeight == null || value.sourceHeight === ''
      ? undefined : Number(value.sourceHeight);
    if (sourceSeconds !== undefined && (!Number.isFinite(sourceSeconds) || sourceSeconds <= 0 || sourceSeconds > 20)) {
      throw new Error('Director extension source duration must be between 0 and 20 seconds');
    }
    for (const [dimension, label] of [[sourceWidth, 'width'], [sourceHeight, 'height']]) {
      if (dimension !== undefined && (!Number.isSafeInteger(dimension) || dimension < 16 || dimension > 8192)) {
        throw new Error(`Director extension source ${label} must be between 16 and 8,192 pixels`);
      }
    }
    if ((sourceWidth === undefined) !== (sourceHeight === undefined)) {
      throw new Error('Director extension source width and height must be provided together');
    }
    return {
      inputName,
      fileName: String(value.fileName || value.label || path.posix.basename(inputName)).trim().slice(0, 255) || 'Video',
      sourceSeconds,
      sourceWidth,
      sourceHeight,
      sourceHasAudio: value.sourceHasAudio === true,
...
async function buildLtxDirectorGraph(project, opts, settings, helpers) {
  ...
  if (extension) {
    ...
    // Decode only the final source frame for continuity. The encoded source
    // and generated tail are joined after generation, avoiding multi-gigabyte
    // full-video IMAGE batches in ComfyUI.
    graph.extension_source = await nodeFromOrdered('VHS_LoadVideo', [], {}, {
      ...
      frame_load_cap: 1,
      skip_first_frames: Math.max(0, plan.sourceOriginalFrames - 1),
      select_every_nth: 1,
      format: 'None',
    });
    graph.extension_tail_resize = {
      ...
      inputs: {
        image: ['extension_source', 0], upscale_method: 'lanczos',
  ...
  if (extension) {
    ...
    frameSource = ['extension_tail_frames', 0];
