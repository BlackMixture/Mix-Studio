...
function nodeLabelForJob(job, nodeId) {
  ...
  if (classType === 'UNETLoader' && job?.kind === 'video' && job?.videoInfo?.engine === 'h3') {
    return 'Loading MiniMax H3...';
  }
  if (classType === 'UNETLoader' && job?.kind === 'video' && job?.videoInfo?.engine === 'wan-animate2') {
    return 'Loading Wan Animate 2...';
  }
  if (SHARED_SAMPLERS.has(classType)) {
    const phase = progressPhaseForJob(job, nodeId);
    if (job && job.kind === 'video' && job.videoChunkSequence && phase.isSampling) {
      return `${phase.phaseLabel} ${phase.phaseIndex} of ${phase.phaseCount}`;
    }
    if (job && job.kind === 'loraHunt' && phase.phaseCount > 1) {
      return `Strength comparison ${phase.phaseIndex} of ${phase.phaseCount}`;
    }
...
function samplerPhaseLabel(job, nodeId, index, count) {
  if (job && job.kind === 'loraHunt') {
    const variant = job.huntPlan && job.huntPlan.variants && job.huntPlan.variants[index];
    return variant && variant.label ? variant.label : `Comparison ${index + 1}`;
  }
...
function progressPhaseForJob(job, nodeId) {
  ...
  if (phase && job?.videoChunkSequence) {
    const sequenceType = job.videoChunkSequence.type;
    const phaseLabel = sequenceType === 'wan-animate2'
      ? 'Wan Animate 2 clip'
      : (sequenceType === 'long-context' ? 'H3 Long context clip' : 'H3 Turbo chunk');
    return {
      isSampling: true,
      nodeId: phase.nodeId,
      phaseIndex: job.videoChunkSequence.index + 1,
      phaseCount: job.videoChunkSequence.segments.length,
      phaseLabel,
    };
  }
...
function progressDetailsForJob(job, nodeId, value, max) {
  ...
  const phase = progressPhaseForJob(job, nodeId);
  const reserveFinalization = job?.kind === 'video' && phase.isSampling;
  const aggregateStrengthHunt = job?.kind === 'loraHunt' && phase.phaseCount >= 2;
  if (!reserveFinalization && !aggregateStrengthHunt) {
