...
function progressPhaseForJob(job, nodeId) {
  ...
  if (phase && job?.videoChunkSequence) {
    const longContext = job.videoChunkSequence.type === 'long-context';
    return {
      isSampling: true,
      nodeId: phase.nodeId,
      phaseIndex: job.videoChunkSequence.index + 1,
      phaseCount: job.videoChunkSequence.segments.length,
      phaseLabel: longContext ? 'H3 Long context clip' : 'H3 Turbo chunk',
    };
  }
...
function progressDetailsForJob(job, nodeId, value, max) {
  ...
  const phase = progressPhaseForJob(job, nodeId);
  if (!phase.isSampling || !job || !['video', 'loraHunt'].includes(job.kind) || phase.phaseCount < 2) {
