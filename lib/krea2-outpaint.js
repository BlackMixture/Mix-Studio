  ...
  if (params.composite === true) {
    graph.preserve_source = {
      ...
      inputs: {
        destination: output,
        source,
        x: Math.max(0, Math.round(padding.left)),
        y: Math.max(0, Math.round(padding.top)),
        resize_source: false,
      },
    };
    output = ['preserve_source', 0];
  }
  graph.save = { class_type: 'SaveImage', inputs: { images: output, filename_prefix: 'KreaStudio/outpaint' } };
  return graph;
}
