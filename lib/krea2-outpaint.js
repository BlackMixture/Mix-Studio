...
function calculateNativeOutpaintPlan({
  ...
  let finalHeight;
  if (axis === 'horizontal') {
    finalHeight = Math.max(sh, Math.ceil(sh / factor));
    finalWidth = Math.max(sw, Math.ceil(finalHeight * targetRatio));
  } else {
    finalWidth = Math.max(sw, Math.ceil(sw / factor));
    finalHeight = Math.max(sh, Math.ceil(finalWidth / targetRatio));
  }

  let limited = false;
  if (finalWidth * finalHeight > maxFinalPixels) {
    const shrink = Math.sqrt(maxFinalPixels / (finalWidth * finalHeight));
    finalWidth = Math.max(sw, Math.floor(finalWidth * shrink));
    finalHeight = Math.max(sh, Math.floor(finalHeight * shrink));
    limited = true;
  }
