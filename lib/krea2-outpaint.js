...
function buildKrea2OutpaintGraph(params = {}) {
  ...
  if (identityEnabled) {
    graph.identity_lora = {
      ...
      inputs: {
        ...
        strength_model: identityOverride
          ? Math.max(-100, Math.min(100, Number.isFinite(Number(identityOverride.strength)) ? Number(identityOverride.strength) : 1))
          : 1,
      },
    };
  }
    ...
    .forEach((lora, index) => {
      ...
      graph[key] = {
        ...
        inputs: {
          model,
          lora_name: lora.name,
          strength_model: Math.max(-100, Math.min(100, Number.isFinite(Number(lora.strength)) ? Number(lora.strength) : 1)),
        },
      };
      model = [key, 0];
    });
  ...
  graph.model_patch = {
    ...
    inputs: {
      ...
      ref_boost: Math.max(0, Math.min(20,
        Number.isFinite(Number(params.krea2RefBoost)) ? Number(params.krea2RefBoost) : 4)),
      ref_boost_a: 1,
      fit_mode: 'fit',
      vae: ['vae', 0],
      source_image: ['scaled_source', 0],
    },
  };
  graph.positive = {
  ...
  graph.sampler = {
    ...
    inputs: {
      ...
      steps: Math.max(8, Math.min(12, Math.round(Number(params.steps) || 10))),
      cfg: 1,
      sampler_name: 'euler',
      scheduler: 'simple',
      denoise: 1,
