...
function calculateOutpaintLayout({ sourceWidth, sourceHeight, targetWidth, targetHeight, position, scale = 1 }) {
  ...
  const placement = normalizeOutpaintPosition(position);
  const horizontal = splitPadding(tw - resizedWidth, axis === 'horizontal' ? placement : 'center');
  const vertical = splitPadding(th - resizedHeight, axis === 'vertical' ? placement : 'center');
  return {
...
function exactSourcePadding(sourceWidth, sourceHeight, targetWidth, targetHeight, axis, position) {
  const placement = normalizeOutpaintPosition(position);
  const horizontal = splitPadding(
    Math.max(0, Math.round(targetWidth) - Math.round(sourceWidth)),
    axis === 'horizontal' ? placement : 'center'
  );
  const vertical = splitPadding(
    Math.max(0, Math.round(targetHeight) - Math.round(sourceHeight)),
    axis === 'vertical' ? placement : 'center'
  );
  return {
...
function calculateNativeOutpaintPlan({
  ...
  if (finalWidth < sw || finalHeight < sh) {
    throw new Error('The native source is too large for the maximum outpaint canvas. Increase Image size or reduce the source resolution.');
  }

  const working = normalizeOutpaintDimensions(finalWidth, finalHeight, maxWorkingPixels);
  const workingScale = Math.min(working.width / finalWidth, working.height / finalHeight);
  const workingSourceWidth = Math.min(working.width, roundSource16(sw * workingScale));
  const workingSourceHeight = Math.min(working.height, roundSource16(sh * workingScale));
  const finalPadding = exactSourcePadding(sw, sh, finalWidth, finalHeight, axis, position);
  const workingPadding = exactSourcePadding(
    workingSourceWidth,
    workingSourceHeight,
    working.width,
    working.height,
