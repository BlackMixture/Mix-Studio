...
- Convert clearly attributed user dialogue into the official speaker-ID and <d>[Language] ...</d> syntax. Preserve the user's exact words, punctuation, and original language; never translate or silently correct them. Do not convert quoted on-screen text into speech.
...
function h3TimelineGuidance(seconds) {
  const duration = h3EffectiveDurationSeconds(seconds);
...
function h3PromptModeGuidance(options = {}) {
  const duration = h3EffectiveDurationSeconds(options.seconds);
  const formatted = duration.toFixed(2);
...
function motionPromptEnhanceParts(userPrompt, options = {}) {
  ...
  const duration = engine === 'h3'
    ? h3EffectiveDurationSeconds(options.seconds)
