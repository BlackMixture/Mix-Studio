...
const H3_MOTION_INSTRUCTION = `Look at the provided first-frame image. Write one compact prompt block for MiniMax H3 image-to-video. Keep the visible subject, identity, setting, and composition faithful to the frame. Begin with the overall action, then describe plausible chronological subject and secondary motion across the requested duration. Use timed beats only when more than one distinct beat improves the shot. Include useful camera behavior and native audio direction in the same block: ambience, sound effects, and music; preserve exact quoted dialogue when the user supplies it, but never invent speech. Do not re-describe static appearance. Keep the finished prompt under 120 words.`;
