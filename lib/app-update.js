...
function restartRequiredForFiles(files) {
  return (files || []).some((file) => (
    ...
    file === 'public/h3-prompt-guide.js' ||
    file.startsWith('lib/')
  ));
}
...
async function updateFromGit(root, options = {}) {
  ...
  const inspectAssets = options.inspectAssets || (() => inspectCriticalPublicAssets(root));
  ...
  if (!assets?.ok) {
    ...
    const error = new Error(`The update is incomplete because critical app files are missing${missing.length ? `: ${missing.join(', ')}` : ''}.`);
    error.code = 'update_assets_missing';
