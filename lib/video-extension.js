...
const fs = require('fs');
const { MAX_INPUT_BYTES, inputAssetPath } = require('./input-assets');
...
const DURABLE_UPLOADED_VIDEO_RE = /^ks_\d{10,17}_[0-9a-f]{8}_[^/\\]{1,460}\.(?:mp4|webm|mov)$/i;
...
function extensionSourceError(message, code) {
...
/**
 * Resolve only files created by the durable /api/upload path. The random
 * server-issued name is part of the capability; arbitrary ComfyUI inputs and
 * filesystem-looking names are deliberately rejected.
 */
async function resolveDurableUploadedVideo(inputDirectory, inputName, options = {}) {
  const name = String(inputName || '').trim();
  if (!DURABLE_UPLOADED_VIDEO_RE.test(name)) {
    throw extensionSourceError('The uploaded extension source is invalid.', 'invalid_extension_source');
  }
  const file = inputAssetPath(inputDirectory, name);
  const fsp = options.fsp || fs.promises;
  let stat;
  try {
    stat = await fsp.stat(file);
  } catch {
    throw extensionSourceError('The uploaded video is no longer available.', 'missing_extension_source');
  }
  if (!stat || (typeof stat.isFile === 'function' && !stat.isFile()) || !(Number(stat.size) > 0)) {
    throw extensionSourceError('The uploaded video is no longer available.', 'missing_extension_source');
  }
  if (Number(stat.size) > MAX_INPUT_BYTES) {
    throw extensionSourceError('The uploaded video exceeds the 2 GB input limit.', 'invalid_extension_source');
  }
  return { name, file, size: Number(stat.size) };
}
...
function extensionDimensions(width, height, fourK) {
  ...
  const upscale = (fourK === true || Math.max(sourceWidth, sourceHeight) > 1536)
  ...
  const outputWidth = Math.max(2, sourceWidth - (sourceWidth % 2));
  const outputHeight = Math.max(2, sourceHeight - (sourceHeight % 2));
  return { W, H, outputWidth, outputHeight, fourK: upscale };
}
...
function normalizeDirectorExtensionPlan(input = {}) {
  ...
  const recordedSourceFrames = Math.round(finiteNumber(info.frames, 0));
  ...
  const legacySmooth = [2, 3, 4].includes(Number(info.smooth)) ? Number(info.smooth) : 1;
  ...
  const sourceSpanSeconds = Math.max(0, sourceFrameCount - 1) / sourceFps;
  ...
  return Object.assign({}, dimensions, {
    ...
    normalizedSeconds,
    sourceSeconds,
    sourceOriginalFrames: sourceFrameCount,
    sourceFrames,
    continuationFrames,
    appendedFrames,
    totalFrames: sourceFrames + appendedFrames,
