...
function normalizeGithubRelease(value, options = {}) {
  if (!value || typeof value !== 'object' || value.draft || (value.prerelease && !options.preview)) return null;
  const tagName = cleanSingleLine(value.tag_name, 64);
  const parsed = parseSemver(tagName);
  if (!tagName || !parsed || (!options.preview && parsed.prerelease.length)) return null;
  return {
    id: tagName,
    tagName,
...
function createGithubReleaseChecker(options = {}) {
  ...
  async function fetchLatest() {
    ...
    let response;
    try {
      response = await fetchImpl(preview ? `https://api.github.com/repos/${OFFICIAL_REPOSITORY}/releases?per_page=100` : LATEST_RELEASE_API, {
        headers: {
    ...
    if (response.status !== 404) {
      ...
      latest = preview && Array.isArray(payload)
        ? payload.map((entry) => normalizeGithubRelease(entry, { preview: true })).filter(Boolean)
          .sort((a, b) => compareSemver(b.version, a.version))[0] || null
        : normalizeGithubRelease(payload);
      if (preview && Array.isArray(payload) && !latest) { cachedRelease = null; cachedAt = now(); hasCache = true; return { latest: null, checkedAt: cachedAt }; }
