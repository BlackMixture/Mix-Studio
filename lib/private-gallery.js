...
function parseCookies(header) {
  ...
  for (const part of String(header || '').split(';')) {
    ...
    // Cookies ignore ports, so other localhost apps (ComfyUI, AI-Toolkit, …)
    // can plant values with malformed percent-sequences. One bad cookie must
    // not take down the whole request — keep the raw value instead.
    try {
