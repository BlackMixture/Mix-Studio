...
function kleinConsistencyConfig(params, settings) {
  ...
  return Object.assign(configured, {
    ...
    strength: override ? clamp(override.strength, -100, 100, 0.6) : 0.6,
  });
}
  ...
  for (const lora of Array.isArray(loras) ? loras : []) {
    ...
    graph[key] = {
      ...
      inputs: {
        ...
        lora_name: lora.name,
        strength_model: clamp(lora.strength, -100, 100, 1),
        strength_clip: clamp(lora.strength, -100, 100, 1),
      },
    };
    model = [key, 0];
    clip = [key, 1];
  ...
  if (preset.lightning && (!lightningOverride || lightningOverride.on !== false)) {
    graph.lightning = {
      ...
      inputs: {
        ...
        strength_model: lightningOverride ? clamp(lightningOverride.strength, -100, 100, 1) : 1,
      },
    };
    model = ['lightning', 0];
  }
