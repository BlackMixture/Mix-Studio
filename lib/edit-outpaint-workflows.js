...
function appendPreservedSource(graph, generated, params, key = 'preserve_source') {
  ...
  graph[key] = {
    ...
    inputs: {
      ...
      source: graph.resized_source ? ['resized_source', 0] : ['source', 0],
      x: Math.max(0, Math.round(padding.left || 0)),
      y: Math.max(0, Math.round(padding.top || 0)),
      resize_source: false,
    },
  };
  return [key, 0];
...
function appendColorMatchedOutput(graph, decoded, prefix, params) {
  graph.color_match = {
  ...
  const output = appendPreservedSource(graph, ['color_match', 0], params);
  graph.save = {
    class_type: 'SaveImage',
    inputs: { images: output, filename_prefix: prefix },
  };
}
...
function buildKleinOutpaintGraph(params = {}) {
  ...
  appendColorMatchedOutput(graph, ['decode', 0], 'KreaStudio/outpaint-klein', params);
  return graph;
}
...
function buildQwenOutpaintGraph(params = {}) {
  ...
  graph.decode = { class_type: 'VAEDecode', inputs: { samples: ['sampler', 0], vae: ['vae', 0] } };
  appendColorMatchedOutput(graph, ['decode', 0], 'KreaStudio/outpaint-qwen', params);
  return graph;
}
...
function buildKrea2MaskedOutpaintGraph(params = {}) {
  ...
  graph.save = { class_type: 'SaveImage', inputs: { images: output, filename_prefix: 'KreaStudio/outpaint-krea2' } };
  return graph;
}
