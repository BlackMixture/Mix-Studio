...
function buildKleinOutpaintGraph(params = {}) {
  ...
  if (consistency.name) {
    graph.klein_outpaint_consistency = {
      class_type: 'LoraLoaderModelOnly',
      inputs: { model, lora_name: consistency.name, strength_model: 0.6 },
    };
  ...
  graph.positive_text = {
    ...
    inputs: { clip: ['clip', 0], text: appendTriggerPhrase(greenOutpaintPrompt(params.prompt), consistency.trigger) },
  };
