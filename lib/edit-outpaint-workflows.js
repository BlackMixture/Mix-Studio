...
function appendPaddedGreenSource(graph, params) {
  ...
  const preserveMaskName = String(params.maskImageName || '').trim();
  if (preserveMaskName && sourceWidth > 0 && sourceHeight > 0) {
    graph.outpaint_keep_mask_load = { class_type: 'LoadImage', inputs: { image: preserveMaskName } };
    graph.outpaint_keep_mask = {
      class_type: 'ImageToMask',
      inputs: { image: ['outpaint_keep_mask_load', 0], channel: 'red' },
    };
    graph.outpaint_keep_mask_image = { class_type: 'MaskToImage', inputs: { mask: ['outpaint_keep_mask', 0] } };
    graph.outpaint_keep_mask_scaled_image = {
      class_type: 'ImageScale',
      inputs: {
        image: ['outpaint_keep_mask_image', 0],
    ...
    graph.outpaint_keep_mask_scaled = {
      class_type: 'ImageToMask',
      inputs: { image: ['outpaint_keep_mask_scaled_image', 0], channel: 'red' },
    };
    graph.outpaint_regenerate_mask = {
      class_type: 'InvertMask',
      inputs: { mask: ['outpaint_keep_mask_scaled', 0] },
    };
    graph.subject_source = {
      class_type: 'DrawMaskOnImage',
      inputs: { image: source, mask: ['outpaint_regenerate_mask', 0], color: '0, 255, 0', device: 'cpu' },
    };
    source = ['subject_source', 0];
  }
  graph.padded = {
...
function buildKrea2MaskedOutpaintGraph(params = {}) {
  ...
  let generationMask = ['scaled_mask', 0];
  const preserveMaskName = String(params.maskImageName || '').trim();
  if (preserveMaskName && sourceWidth > 0 && sourceHeight > 0) {
    graph.regenerate_mask_load = { class_type: 'LoadImage', inputs: { image: preserveMaskName } };
    graph.regenerate_keep_mask = {
      class_type: 'ImageToMask',
      inputs: { image: ['regenerate_mask_load', 0], channel: 'red' },
    };
    graph.regenerate_keep_image = { class_type: 'MaskToImage', inputs: { mask: ['regenerate_keep_mask', 0] } };
    graph.regenerate_keep_scaled_image = {
      class_type: 'ImageScale',
      inputs: {
        image: ['regenerate_keep_image', 0],
        upscale_method: 'lanczos',
        width: sourceWidth,
        height: sourceHeight,
        crop: 'disabled',
    ...
    graph.regenerate_keep_scaled = {
      class_type: 'ImageToMask',
      inputs: { image: ['regenerate_keep_scaled_image', 0], channel: 'red' },
    };
    graph.regenerate_background_mask = {
      class_type: 'InvertMask',
      inputs: { mask: ['regenerate_keep_scaled', 0] },
    };
    graph.regenerate_background_image = {
      class_type: 'MaskToImage',
      inputs: { mask: ['regenerate_background_mask', 0] },
    };
    graph.regenerate_background_padded_image = {
      class_type: 'ImagePadForOutpaint',
      inputs: {
        image: ['regenerate_background_image', 0],
        left: Math.max(0, Math.round(padding.left || 0)),
        top: Math.max(0, Math.round(padding.top || 0)),
        right: Math.max(0, Math.round(padding.right || 0)),
        bottom: Math.max(0, Math.round(padding.bottom || 0)),
    ...
    graph.regenerate_background_canvas_mask = {
      class_type: 'ImageToMask',
      inputs: { image: ['regenerate_background_padded_image', 0], channel: 'red' },
    };
    graph.combined_outpaint_mask = {
      class_type: 'MaskComposite',
      inputs: {
        destination: ['scaled_mask', 0],
        source: ['regenerate_background_canvas_mask', 0],
        x: 0,
        y: 0,
        operation: 'add',
      },
    };
    generationMask = ['combined_outpaint_mask', 0];
  }
  ...
  graph.masked_latent = { class_type: 'SetLatentNoiseMask', inputs: { samples: ['encode', 0], mask: generationMask } };
  graph.sampler = {
    class_type: 'KSampler',
    inputs: {
      model: chain.model,
